...
sgs.ai_cardneed.jielieren = function(to, card, self)
...
sgs.ai_skill_invoke.jielieren = function(self, data)
	...
	for _, target in sgs.qlist(use.to) do--
		...
		if (self.player:getHandcardNum() >= self.player:getHp() or self:getMaxNumberCard():getNumber() > 10
			...
			and not self:doNotDiscard(target, "h", true) and not (self.player:getHandcardNum() == 1 and 
			self:doNotDiscard(target, "e", true)) then--
				return true
		end
...
function sgs.ai_skill_pindian.jielieren(minusecard, self, requestor)
	local cards = sgs.QList2Table(self.player:getHandcards())
	self:sortByKeepValue(cards)
...
sgs.ai_skill_playerchosen.shushen = function(self, targets)
	...
	for _, p in sgs.qlist(friendsByAction) do
		if self.player:isFriendWith(p) and not self:needKongcheng(p) and self.player:objectName() ~= p:objectName() then
			return p
		end
	end
	for _, p in sgs.qlist(friendsByAction) do
		if self:isFriend(p) and not self:needKongcheng(p) and self.player:objectName() ~= p:objectName() then
			return p
		end
	end
