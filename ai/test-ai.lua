...
sgs.ai_use_priority.jiejingheCard = 8.9
...
sgs.ai_skill_invoke.jiepolu = function(self, data)
	...
	if yuanshu and self:isEnemy(yuanshu) and yuanshu:getPhase() <= sgs.Player_Play and not yuanshu:hasUsed("#WeidiCardrd")
    then return false end
    if #self.friends_noself > 0 or self.player:isAlive() then return true end
    return false
end
...
jiezhengbing_skill.getTurnUseCard = function(self, inclusive)
    ...
    if self:getCardsNum("Slash") > 1 and not hasCross then
...
sgs.ai_skill_invoke.jiefenwei = function(self, data)
    ...
    elseif jiefenwei_value >= 3 then --队友至少3名时
        ...
        --[[elseif card:isKindOf("FightTogether") and use.to:contains(self.player) then --这个会闪退
            for _, p in pairs(tos) do
...
sgs.ai_skill_playerchosen.jiefenwei = function(self, targets)
    if self.jiefenweiTarget then return self.jiefenweiTarget end
    return {}
end
...
sgs.ai_skill_playerchosen["jiediaodu_take"] = function(self, targets)
    for _, hcard in sgs.qlist(self.player:getCards("e")) do
        ...
        elseif hcard:objectName() == "SilverLion" and self.player:getLostHp() >= 1 then
            return self.room:getCurrent()
        elseif self.player:hasShownSkills(sgs.lose_equip_skill) then
            return self.room:getCurrent()
        end
    end
    for _, p in sgs.qlist(targets) do
    ...
    if self.player:hasShownSkills(sgs.lose_equip_skill) and self.player:getCards("e"):length() == 0 then
        for _, p in sgs.qlist(targets) do
...
sgs.ai_skill_playerchosen["jiediaodu_give"] = function(self, targets)
    local diaodu_card
    if self.diaodu_id then
        diaodu_card = sgs.Sanguosha:getCard(self.diaodu_id)
    end
    for _, p in sgs.qlist(targets) do
        if p:hasShownSkills(sgs.lose_equip_skill) and self.player:isFriendWith(p) and not self:willSkipPlayPhase(p) then
            return p
        end
    end
...
sgs.ai_skill_choice.jiediaodu = function(self, choices, data)
	return "yes"
end
...
sgs.ai_skill_choice["luaxuyuan_choice"] = function(self, choices, data)
    ...
    if self.player:isFriendWith(current) and not self.player:isNude() then
        if current:hasShownSkills("yongsi|fenglve|shenwei|zhuidu") then
...
sgs.ai_skill_invoke.luashibei = function(self, data)
    if self.player:getLostHp() <= 0 then return false end
    local damage = data:toDamage()
    local from = damage.from
    local equipcards_from = nil
    local equipcards_self = self.player:getCards("he")
    local hasHorse_self, hasHorse_from = false, false
    if from then
        equipcards_from = from:getCards("e")
    ...
    if hasHorse_self then
        return true
    elseif hasHorse_from and (from and from:objectName() ~= self.player:objectName() and not from:hasShownSkills(sgs.lose_equip_skill) or (from:hasShownSkill("xiaoji") and 
    from:getPhase() ~= sgs.Player_NotActive)) then
        return true
    end
    return false
end
sgs.ai_skill_choice.luashibei = function(self, choices, data)
    ...
    local equipcards_from = from:getCards("e")
    local equipcards_self = self.player:getCards("he")
    ...
    for _, card in sgs.qlist(equipcards_self) do
        if card:isKindOf("OffensiveHorse") or card:isKindOf("DefensiveHorse")  or card:isKindOf("SixDragons") then
            hasHorse_self = true
            self.luashibeiCard1 = card:getId()
            break
        end
    end
    for _, card in sgs.qlist(equipcards_from) do
        if card:isKindOf("OffensiveHorse") or card:isKindOf("DefensiveHorse")  or card:isKindOf("SixDragons") then
            hasHorse_from = true
            self.luashibeiCard2 = card:getId()
            break
        end
    end
    if self.player:getHp() == 1 and hasHorse_self then
        return "luashibei_discard"
    elseif hasHorse_from and not from:hasShownSkill("xuanlue") and not (from:hasShownSkill("xiaoji") and from:getPhase() == 
...
sgs.ai_skill_cardchosen.luashibei = function(self, who, flags, method, disable_list)
...
sgs.ai_skill_use["@@luashibeiVS"] = function(self, prompt)
    for _, card in sgs.qlist(self.player:getCards("he")) do
        if card:isKindOf("OffensiveHorse") or card:isKindOf("DefensiveHorse")  or card:isKindOf("SixDragons") then
            return "#luashibeiCard:" .. card:getId() .. ":&luashibei"
        end
    end
    return "."
end
...
luafenyue_skill.getTurnUseCard = function(self, inclusive)
    ...
    if self.player:hasFlag("luafenyue_fail") or self.player:isKongcheng() or self.player:usedTimes("ViewAsSkill_luafenyueCard") 
...
sgs.ai_skill_use_func["#luafenyueCard"] = function(card, use, self)
    for _, enemy in pairs(self.enemies) do
        if not enemy:isKongcheng() and not enemy:isRemoved() and not enemy:hasArmorEffect("PeaceSpell") then
            if not (enemy:hasShownSkill("liuli") and enemy:hasShownSkill("xuanlue|xiaoji")) or not enemy:hasShownSkill("xiangle") then
    ...
    if not target then
        if sec_target then
            ...
            for i = 1, self.player:distanceTo(sec_target) do
                if not (self.player:isBigKingdomPlayer() and cards[i]:isKindOf("ThreatenEmperor")) then
...
sgs.ai_skill_invoke.luachengxu = function(self, data)
    if self.player:hasFlag("luachengxu2slash") then
        ...
        if not self:isFriend(target) and self:damageIsEffective(target, nil, self.player) then
            ...
            if target:hasShownSkill("jiebuqu") and target:getPile("scars"):length() <= 4 then 
...
sgs.ai_skill_choice.luachengxu = function(self, choices, data)
    ...
    if (self.player:isFriendWith(target) or self.player:willBeFriendWith(target)) and not (target:hasShownSkill("leiji") and 
    (target:getHandcardNum() > 3 or target:hasArmorEffect("EightDiagram"))) then return "no" end
    if not self:isFriend(target) and target:hasShownSkill("leiji") and (target:getHandcardNum() > 3 or 
    target:hasArmorEffect("EightDiagram")) then return "no" end
	return "yes"
end
...
sgs.ai_skill_playerchosen.luabingzheng = function(self, targets)
    if self.player:hasShownSkill("kongcheng") and self.player:getHandcardNum() == 1 and self.player:getHp() > 1 then
        for _, p in pairs(targets) do
            if p:objectName() == self.player:objectName() then
    ...
    for _, p in pairs(targets) do
        if not self:isFriend(p) and p:getHandcardNum() - 1 == p:getHp() and not p:hasShownSkill("lirang") then
            self.luabingzhengchoice = "luabingzhengDiscard"
            return p
        end
    end
    ...
    for _, p in pairs(targets) do
        ...
        if not self:isFriend(p) and p:getHandcardNum() < p:getHp() and not self:needKongcheng(p) and not p:isKongcheng()
        and not p:hasShownSkill("lirang") then
            self.luabingzhengchoice = "luabingzhengDiscard"
            return p
        end
        ...
        if not self:isFriend(p) and not self:needKongcheng(p) and not p:isKongcheng() and not p:hasShownSkill("lirang") then
            self.luabingzhengchoice = "luabingzhengDiscard"
            return p
        end
    end
...
sgs.ai_skill_choice.luabingzheng = function(self, choices, data)
    local target = self.player:getTag("BingzhengTarget"):toPlayer()
...
sgs.ai_skill_exchange["luabingzheng_give"] = function(self, pattern, max_num, min_num, expand_pile)
    ...
    local target = self.player:getTag("BingzhengTarget"):toPlayer()
    if self:isFriend(target) then
        if target:hasShownSkill("jizhi") then
            for _, c in pairs(hcards) do
                if c:getTypeId() == sgs.Card_TypeTrick and not (c:isKindOf("Lightning") or c:isKindOf("ImperialOrder")) then
                    return c:getEffectiveId()
                end
            end
        elseif target:hasShownSkills("paoxiao|kuangcai|tieqi|qianxi") then
...
sgs.ai_skill_invoke.luasheyan = function(self, data)
    ...
    if card:isKindOf("ExNihilo") then
        ...
        for _, p in sgs.qlist(playersByAction) do
            if self:isFriend(p) and self.player:objectName() ~= p:objectName() and not use.to:contains(p) and 
            not self:needKongcheng(p) then
                self.luasheyanchooseplayer = p
                return true
            end
    ...
    elseif card:isKindOf("IronChain") then
        ...
        if #tos > 1 then
            ...
            for _, p in sgs.qlist(playersByAction) do
                if not table.contains(tos, p) and self.player:isFriendWith(p) and self:trickIsEffective(card, p, from) and p:isChained() then
                    self.luasheyanchooseplayer = p
                    return true
                end
            end
            for _, p in sgs.qlist(playersByAction) do
                if not table.contains(tos, p) and not self:isFriend(p) and self:trickIsEffective(card, p, from) and not p:isChained() and 
                p:canBeChainedBy() then
                    self.luasheyanchooseplayer = p
                    return true
                end
            end
    ...
    elseif card:isKindOf("Duel") then
        ...
        if #tos > 1 then
            for _, p in ipairs(tos) do
                if self.player:isFriendWith(p) and self:trickIsEffective(card, p, from) then
                    self.luasheyanchooseplayer = p
                    return true
                end
            end
        ...
        for _, p in sgs.qlist(playersByAction) do
            if not table.contains(tos, p) and not self:isFriend(p) and from:objectName() ~= p:objectName() and self:trickIsEffective(card, p, from) then
                self.luasheyanchooseplayer = p
                return true
            end
        end
    elseif card:isKindOf("Snatch") or card:isKindOf("Dismantlement") then
        ...
        if is_friend then
            for _, p in sgs.qlist(playersByAction) do
                if not table.contains(tos, p) and self:isFriend(p) and self:trickIsEffective(card, p, from) and not p:getCards("j"):isEmpty() and
            ...
            for _, p in sgs.qlist(playersByAction) do
                ...
                from:objectName() ~= p:objectName() and p:getCards("j"):isEmpty() then
                    if p:hasShownSkills(sgs.lose_equip_skill) and p:isKongcheng() then continue end
                    self.luasheyanchooseplayer = p
                    return true
                end
            end
            ...
            if #tos > 1 then
                ...
                for _, p in sgs.qlist(playersByAction) do
                    if not table.contains(tos, p) and p:isFriendWith(from) and self:trickIsEffective(card, p, from) and p:getCards("j"):isEmpty() and
                ...
                for _, p in sgs.qlist(playersByAction) do
                    if not table.contains(tos, p) and not self:isFriend(p) and self:trickIsEffective(card, p, from) and p:getCards("j"):isEmpty() and
                    not p:isNude() and from:objectName() ~= p:objectName() then
                        if p:hasShownSkills(sgs.lose_equip_skill) and p:isKongcheng() then continue end
                        self.luasheyanchooseplayer = p
                        return true
    ...
    elseif card:isKindOf("LureTiger") then
        local targets = sgs.QList2Table(playersByAction)
        self:sort(targets, "hp")
        for _, p in pairs(targets) do
            if not table.contains(tos, p) and self.player:isFriendWith(p) and self:trickIsEffective(card, p, from) and 
            from:objectName() ~= p:objectName() then
                self.luasheyanchooseplayer = p
                return true
            end
...
sgs.ai_skill_playerchosen.luahuituo = function(self, targets)
    ...
    self:sort(targets, "hp")
    for _, p in pairs(targets) do
        if self.player:isFriendWith(p) and p:getLostHp() > 0 and not p:isRemoved() then
            return p
        end
    end
    for _, p in pairs(targets) do
        if self.player:isFriendWith(p) and not p:isRemoved() then
            return p
        end
    end
    return ""
...
sgs.ai_skill_use_func["#luaxinggongCard"] = function(card, use, self)
    ...
    if not self.luaxinggongPlayer then
        for _, p in pairs(targets) do
            ...
            p:getHp() == 1 and not p:hasShownSkill("jijiu") then
                self.luaxinggongPlayer = p
            end
        end
    end
    if self.luaxinggongPlayer then
        ...
        if use.to then
            ...
            self.luaxinggongPlayer = nil
        end
    end
end

sgs.ai_use_priority.luaxinggongCard = 8.3

sgs.ai_skill_choice.luaxinggong = function(self, choices, data)
    local target = data:toPlayer()
    if self:isFriend(target) or self.player:willBeFriendWith(target) then return "yes" end
    return "no"
end
...
sgs.ai_skill_use_func["#luazhanjueCard"] = function(card, use, self)
	...
	--[[local cmp = function(a, b)
		...
		if not self:isWeak(a) and a:hasShownSkill("jianxiong") then v1 = v1 + 10 end
		if not self:isWeak(b) and b:hasShownSkill("jianxiong") then v2 = v2 + 10 end
		...
		if not self:isWeak(a) and a:hasShownSkill("jiang") then v1 = v1 + 5 end
...
sgs.ai_skill_invoke.luazhuanxing = function(self, data)
    ...
    if slashNum > 0 and not isSelf then
        if current:hasShownSkill("zhente") then
            for _, slash in pairs(slashCards) do
...
sgs.ai_skill_use["@@luazhuanxingShortage"] = function(self, prompt, method)
    if self.luazhuanxingShortage then
        local cardid = self.luazhuanxingShortage
        self.luazhuanxingShortage = nil
        return "#luazhuanxingCard:" .. cardid .. ":&luazhuanxing"
    end
    return ""
end
...
luachenyin_skill.getTurnUseCard = function(self, inclusive)
    ...
    if (self:getOverflow() - 2 >= 0) or not (self:getCardsNum("Peach") > 0 and self.player:getHandcardNum() <= 2) then
        local hasThreaten = false
        local hcard = self.player:getCards("h")
        for _, c in sgs.qlist(hcard) do
            if c:isKindOf("ThreatenEmperor") then
        ...
        if not (hasThreaten and self.player:isBigKingdomPlayer()) then
            sgs.ai_use_priority.luachenyinCard = 2.1
...
sgs.ai_skill_use_func["#luachenyinCard"] = function(card, use, self)
    if self.luachenyinTarget then
        use.card = card
        local target = self.luachenyinTarget
        if use.to then
...
sgs.ai_skill_invoke.luazifeng = function(self, data)
    ...
    if self.player:isRemoved() and not self:willSkipPlayPhase() then
        return false
    end
    return true
end
...
sgs.ai_skill_playerchosen.luazuoding = function(self, targets)
    if self.luazuodingTarget then return self.luazuodingTarget end
    return ""
end
...
luahuomo_skill.getTurnUseCard = function(self, inclusive)
    ...
    local notBasic = {}
    for _, c in sgs.qlist(hecards) do
        if c:isBlack() and c:getTypeId() ~= sgs.Card_TypeBasic then
    ...
    if self.player:getLostHp() > 0 then
        local skillcard = sgs.Card_Parse("peach:luahuomo[no_suit:0]=" .. c:getEffectiveId() .. "&luahuomo")
        assert(skillcard)
        return skillcard
    end
    if self:getCardsNum("Slash") > 0 then
        for _, enemy in pairs(self.enemies) do
            if sgs.isGoodTarget(enemy, self.enemies, self) and self.player:inMyAttackRange(enemy) then
                local skillcard = sgs.Card_Parse("analeptic:luahuomo[no_suit:0]=" .. c:getEffectiveId() .. "&luahuomo")
                assert(skillcard)
                return skillcard
            end
        end
            ...
            for _, enemy in pairs(self.enemies) do
                if self.player:canSlash(enemy, true) and sgs.isGoodTarget(enemy, self.enemies, self) and self:inMyAttackRange(enemy) then
                    if enemy:hasArmorEffect("Vine") then
                        local skillcard = sgs.Card_Parse("fire_slash:luahuomo[no_suit:0]=" .. c:getEffectiveId() .. "&luahuomo")
                        assert(skillcard)
                        return skillcard
                    else
                        local skillcard = sgs.Card_Parse("slash:luahuomo[no_suit:0]=" .. c:getEffectiveId() .. "&luahuomo")
                        assert(skillcard)
                        return skillcard
                    end
                end
...
sgs.ai_cardsview.luahuomo = function(self, class_name, player)
	...
	if class_name == "Analeptic" then
        return ("analeptic:luahuomo[no_suit:0]=" .. notBasic[1] .. "&luahuomo")
	end
    if class_name == "Peach" then
        return ("peach:luahuomo[no_suit:0]=" .. notBasic[1] .. "&luahuomo")
    end
    if class_name == "Jink" then
        return ("jink:luahuomo[no_suit:0]=" .. notBasic[1] .. "&luahuomo")
    end
    if class_name == "Slash" then
        return ("slash:luahuomo[no_suit:0]=" .. notBasic[1] .. "&luahuomo")
    end
    return
end
--钟繇：出牌阶段优先发动【活墨】，处理完活墨之后再按照原来的出牌顺序用其余的牌
--（活墨一旦在本阶段失去过牌就会失效，必须排在其它用牌之前）
local luahuomo_getTurnUse = SmartAI.getTurnUse
function SmartAI:getTurnUse()
    local cards = luahuomo_getTurnUse(self)
    if self.player:hasSkill("luahuomo") and not self.player:hasFlag("luahuomo_lose") then
        local has_huomo = false
        for _, card in ipairs(cards) do--活墨卡已经被正常收集
            if card:getSkillName() == "luahuomo" then
                has_huomo = true
                break
            end
        end
        if not has_huomo then--getTurnUse 遇到优先级>=10的牌会提前结束遍历，这里补一张进队首
            local skillcard = luahuomo_skill.getTurnUseCard(self, false)
            if skillcard then table.insert(cards, 1, skillcard) end
        end
    end
    return cards
end

local luahuomo_sortByDynamicUsePriority = SmartAI.sortByDynamicUsePriority
function SmartAI:sortByDynamicUsePriority(cards)
    luahuomo_sortByDynamicUsePriority(self, cards)--先按原来的优先级排序
    if not self.player or not self.player:hasSkill("luahuomo") or self.player:hasFlag("luahuomo_lose") then return end
    local huomo, others = {}, {}
    for _, card in ipairs(cards) do
        if card:getSkillName() == "luahuomo" then
            table.insert(huomo, card)
        else
            table.insert(others, card)
        end
    end
    if #huomo == 0 then return end
    local index = 0--活墨转化的牌整体提到最前，其内部与其它牌的顺序保持不变
    for _, card in ipairs(huomo) do
        index = index + 1
        cards[index] = card
    end
...
sgs.ai_skill_choice["provinceSeal_give"] = function(self, choices, data)
    ...
    local jiangwaifeiyi = sgs.findPlayerByShownSkillName("shoucheng")
    if self.player:hasSkills("kanpo|jijiu|beige") and #hcards < 3 and not (jiangwaifeiyi and self.player:isFriendWith(jiangwaifeiyi)) then return "no" end
    if target:getPhase() <= sgs.Player_Play and not self:willSkipPlayPhase(target) and self.player:isFriendWith(target) then
        ...
        elseif target:hasShownSkill("luayixing") then
            ...
            if armorCard > 0 then
                self.provinceSealCard = armorCard
                return "yes"
            end
        elseif self.player:hasShownSkills(sgs.lose_equip_skill) and self.player:getEquips() then
...
sgs.ai_skill_choice["proSeal_choose1"] = function(self, choices)
    ...
    elseif self.player:getHandcardNum() <= 1 and self.player:getMaxCards() >= 3 then
        ...
        self.proSearlChoose2 = "provinceSeal_reduceMod"
        return "provinceSeal_draw"
    elseif (self.player:getMark("##luajpzzg_killer") > 0 and self:getCardsNum("Slash") <= 1) or self.player:hasSkill("paoxiao|kuangcai") then
        self.proSearlChoose1 = "provinceSeal_add1"
        self.proSearlChoose2 = "provinceSeal_reduceSlash"
        return "provinceSeal_draw"
...
sgs.ai_skill_invoke.luazhudian = function(self, data)
    ...
    local use = data:toCardUse()
    local card = use.card
    if self.player:getHandcardNum() == 1 and self.player:getEquips():isEmpty() then
        local hcard = self.player:getCards("h"):first()
        if (not hcard:isKindOf("Peach") or not hcard:isKindOf("Analeptic")) and not self:isFriend(use.from) then
            if use.card:isKindOf("Slash") and self:slashIsEffective(use.card, self.player, use.from) and not
            hcard:isKindOf("Jink") then
                return false
            elseif use.card:isKindOf("SavageAssault") and self:trickIsEffective(use.card, self.player, use.from) and not
            hcard:isKindOf("Slash") then
                return false
            end
        end
    end
...
luabotong_skill.getTurnUseCard = function(self, inclusive)
    ...
    local dummyc = sgs.DummyCard(Table2IntList(btCards))
    dummyc:deleteLater()
    local dummy = dummyc:getId()
    if self.player:getLostHp() > 0 then
        if self:getCardsNum("Peach") == 0 then
            local skillcard = sgs.Card_Parse("peach:luabotong[no_suit:0]=" .. dummy .. "&luabotong")
            assert(skillcard)
            return skillcard
        else
    ...
    if self:getOverflow() > 2 then
        if sgs.Analeptic_IsAvailable(self.player) then
            local skillcard = sgs.Card_Parse("analeptic:luabotong[no_suit:0]=" .. dummy .. "&luabotong")
            assert(skillcard)
            return skillcard
        end
        if self:slashIsAvailable(self.player) then
            ...
            for _, enemy in pairs(self.enemies) do
                if self.player:canSlash(enemy, true) and sgs.isGoodTarget(enemy, self.enemies, self) and self:inMyAttackRange(enemy) then
                    if enemy:hasArmorEffect("Vine") then
                        local skillcard = sgs.Card_Parse("fire_slash:luabotong[no_suit:0]=" .. dummy .. "&luabotong")
                        assert(skillcard)
                        return skillcard
                    else
                        local skillcard = sgs.Card_Parse("slash:luabotong[no_suit:0]=" .. dummy .. "&luabotong")
                        assert(skillcard)
                        return skillcard
                    end
                end
...
sgs.ai_cardsview.luabotong = function(self, class_name, player)
    ...
    local dummy = dummyc:getId()
    if class_name == "Analeptic" then
		return ("analeptic:luabotong[no_suit:0]=" .. dummy .. "&luabotong")
	end
    if class_name == "Peach" then
        return ("peach:luabotong[no_suit:0]=" .. dummy .. "&luabotong")
    end
    if class_name == "Jink" then
        return ("jink:luabotong[no_suit:0]=" .. dummy .. "&luabotong")
    end
    if class_name == "Slash" then
        return ("slash:luabotong[no_suit:0]=" .. dummy .. "&luabotong")
    end
end
sgs.ai_use_priority.luabotong = 3.2
...
sgs.ai_skill_invoke.luaxiantu = function(self, data)
    ...
    local current = room:getCurrent()
    local hasCross, hasfazheng, hasmin, hasjudge = false, false, false, false
    if current:hasShownSkill("paoxiao") or (current:getWeapon() and current:getWeapon():objectName() == "Crossbow") then 
        hasCross = true
    end
    if not hasCross and not self.player:isNude() then
    ...
    if not current:getJudgingArea():isEmpty() then hasjudge = true end

    if current:hasShownSkills("jizhi|paoxiao|jili") or ((hasfazheng or hasCross) and (current:getHandcardNum() > 5 or 
    current:getMark("@firstshow") > 0)) and hasmin and not current:hasSkill("xuanhuo") then
        self.luaxiantuChoice = "luaxiantuDraw"
    end
    if self.luaxiantuChoice and hasjudge then
        self.luaxiantuChoice = "luaxiantuGet"
    end
    if self.luaxiantuChoice then return true end
    return false
end
sgs.ai_skill_choice.luaxiantu = function(self, choices)
    if self.luaxiantuChoice then return self.luaxiantuChoice end
end
sgs.ai_skill_playerchosen.luaxiantu = function(self, targets)
    return self.room:getCurrent()
end
sgs.ai_skill_cardchosen.luaxiantu = function(self, who, flags, method, disable_list)
    local jcards = who:getCards("j")
    local hasIndu, hasShort, hasLight = false, false, false
    for _, c in sgs.qlist(jcards) do
        if c:isKindOf("Indulgence") then
            hasIndu = true
            self.luaxiantuJcardIndu = c:getId()
        elseif c:isKindOf("SupplyShortage") then
            hasShort = true
            self.luaxiantuJcardShort = c:getId()
        else
            hasLight = true
            self.luaxiantuJcardLight = c:getId()
        end
    end
    if hasIndu then
        return self.luaxiantuJcardIndu
    elseif hasShort then
        return self.luaxiantuJcardShort
    else
        return self.luaxiantuJcardLight
    end
end
sgs.ai_skill_exchange.luaxiantu = function(self, pattern, max_num, min_num, expand_pile)
    local room = self.player:getRoom()
    local current = room:getCurrent()
    local hasCross = false
    local cardsGive = {}
    ...
    if not hasCross then
        for _, c in sgs.qlist(hecards) do
            if c:isKindOf("Crossbow") then
                table.insert(cardsGive, c:getId())
                break
            end
        end
    end
    if current:hasShownSkill("jili") then
        for _, c in sgs.qlist(hecards) do
            if c:getTypeId() == sgs.Card_TypeEquip then
                table.insert(cardsGive, c:getId())
            end
            if #cardsGive == 2 then return cardsGive end
        end
    end
    if current:hasShownSkill("jizhi") then
        for _, c in sgs.qlist(hecards) do
            if c:isNDTrick() and not c:isKindOf("ImperialOrder") then
                table.insert(cardsGive, c:getId())
            end
            if #cardsGive == 2 then return cardsGive end
        end
    ...
    room:sortByUseValue(hecards)
    hecards = sgs.QList2Table(hecards)
    for i = 1, 2 do
        table.insert(cardsGive, hecards[i])
        if #cardsGive == 2 then return cardsGive end
    end
end
...
sgs.ai_skill_use["@@luaqiangzhiUse"] = function(self, prompt)
    ...
    local card_str = (viewcard:getEffectName() .. ":luaqiangzhi[%s:%s]=%d&luaqiangzhi"):format(suit, number, card_id)
    if viewcard:isKindOf("AwaitExhausted") or viewcard:isKindOf("ExNihilo") then
        return card_str
    elseif viewcard:isKindOf("Peach") and self.player:getLostHp() > 0 and self.player:canRecover() then
        return card_str
    elseif viewcard:isKindOf("BefriendAttacking") then
        ...
        local current = room:getCurrent()
        if not self.player:isFriendWith(current) and self:getOverflow(current) > 0 and not current:hasShownSkill("zhiwei") then
            return card_str .. "->" .. current:objectName()
        end
        for i = targets:length() - 1, 0, -1 do
            local p = targets:at(i)
            if self:willSkipPlayPhase(p) and not p:hasShownSkills("luajuxian|hongyan|guicai|fudi|zhendu|shejian") and not self.player:isFriendWith(p) then
                return card_str .. "->" .. p:objectName()
            end
        end
        for i = targets:length() - 1, 0, -1 do
            local p = targets:at(i)
            if not p:hasShownSkills("luajuxian|hongyan|fudi|zhendu|shejian|paoxiao|jizhi|yusui|liuli") and not self.player:isFriendWith(p) then
                return card_str .. "->" .. p:objectName()
            end
        end
    elseif viewcard:isKindOf("SavageAssault") or viewcard:isKindOf("ArcheryAttack") then
        --[[local zhumeng = sgs.findPlayerByShownSkillName("huoshou|juxiang")
        local haszhumeng = false
        if card:isKindOf("SavageAssault") and zhumeng and self.player:isFriendWith(zhumeng) then
            haszhumeng = true
        end]]
        for _, p in pairs(self.friends_noself) do
            if self.player:isFriendWith(p) and self:trickIsEffective(card, p, self.player) then
                if p:getHp() == 1 and (self:getCardsNum("Peach") + self.player:getMark("@companion") <= 0) then
                    return ""
                end
            end
        end
...
--陆逊
sgs.ai_skill_invoke.jieqianxun = function(self, data)
    local effect = data:toCardEffect()
    local card = effect.card
    local source = effect.from
    local hcards = self.player:getCards("h")
    local hasDenfense = false
    local yuanshu = sgs.findPlayerByShownSkillName("weidi")
	if yuanshu and self:isEnemy(yuanshu) and yuanshu:getPhase() <= sgs.Player_Play and not yuanshu:hasUsed("WeidiCard") then
		if card:isKindOf("Snatch") and hcards:length() == 1 and self.player:getEquips():isEmpty() then
            return true
        end
        return false
	end
    if self:getCardsNum("Jink") > 0 or self:getCardsNum("Peach") > 0 or self:getCardsNum("Analeptic") > 0 then
        hasDenfense = true
    end
    if self:willSkipPlayPhase() then
        return true
    ...
    if self:isFriend(source) then
        return true
    elseif not self:isWeak() or (self:isWeak() and not hasDenfense) then
        if (card:isKindOf("Snatch") and (self.player:getEquips():isEmpty() or self.player:getEquips():length() > 1)) or
        card:isKindOf("FireAttack") then
...
sgs.ai_skill_playerchosen.jieqianxun = function(self, targets, max_num, min_num)
    ...
    for _, p in sgs.qlist(targets) do
        if (self.player:isFriendWith(p) or self.player:willBeFriendWith(p)) and not self:needKongcheng(p) then
            ...
            table.insert(drawtargets, p)
        end
        if #drawtargets >= max_num then
            return drawtargets
        end
    end
    if #drawtargets < max_num then
        for _, p in pairs(self.friends_noself) do
            if self:isFriend(p) and not table.contains(drawtargets, p) and not self:needKongcheng(p) then
                table.insert(drawtargets, p)
            end
            if #drawtargets >= max_num then
                break
...
--步骘
sgs.ai_skill_playerchosen.luahongde = sgs.ai_skill_playerchosen.jieshushen

local luadingpan_skill = {}
luadingpan_skill.name = "luadingpan"
table.insert(sgs.ai_skills, luadingpan_skill)
luadingpan_skill.getTurnUseCard = function(self)
    local x = getKingdoms(self.player, false) - 1 --不包含自己的势力
    if self.player:getMark("luadingpanTimes") >= x then return false end
    local room = self.player:getRoom()
    local targets = room:getAlivePlayers()
    local targetslist = {}
    local card_str = "#luadingpanCard:.:&luadingpan"
    for _, p in sgs.qlist(targets) do
        if not p:getEquips():isEmpty() then
            table.insert(targetslist, p)
        end
    end
    for _, p in pairs(targetslist) do
        if self.player:isFriendWith(p) and p:hasShownSkill(sgs.lose_equip_skill) then
            if p:getEquips():length() == 1 and (p:getArmor() or p:getDefensiveHorse() or p:getSpecialHorse() or
            p:getTreasure():objectName() == "JadeSeal") then
                continue
            end
            self.luadingpanTarget = p
            return sgs.Card_Parse(card_str)
        elseif not self:isFriend(p) and p:getArmor() == "PeaceSpell" and not p:hasShownSkill("wendao") and not p:getHp() == 1 then
            self.luadingpanTarget = p
            return sgs.Card_Parse(card_str)
        end
    end
    for _, p in pairs(targetslist) do
        if not self:isFriend(p) and not p:hasShownSkill("paoxiao|zhendu|fudi|kuangcai|kanpo|jili") then
            self.luadingpanTarget = p
            return sgs.Card_Parse(card_str)
        end
    end
end
sgs.ai_skill_use_func["#luadingpanCard"] = function(card, use, self)
    use.card = card
    if self.luadingpanTarget and use.to then
        use.to:append(self.luadingpanTarget)
    end
end
sgs.ai_use_priority.luadingpanCard = 2.8
sgs.ai_skill_choice["luadingpan"] = function(self, choices)
    if self.player:hasSkill("jili") and self.player:getHp() > 2 and not (self.player:getArmor() and
    self.player:getArmor():objectName() == "PeaceSpell") then
        return "luadingpanGet"
    elseif self.player:hasSkill(sgs.masochism_skill) and self.player:getHp() > 2 or (self.player:getHp() > 1 and
    self:getCardsNum("Peach") > 0) then
        return "luadingpanGet"
    end
...
sgs.ai_skill_choice.jiefenji = function(self, choices, data)
	...
	if yuanshu and self:isEnemy(yuanshu) and yuanshu:getPhase() <= sgs.Player_Play and not 
    yuanshu:hasUsed("#WeidiCardrd") then return false end
    if self.player:isRemoved() and self.player:isFriendWith(target) then
        return "yes"
    end
    if (self.player:isFriendWith(target) or self.player:willBeFriendWith(target)) and (len < 3 or self.player:getHp() > 2) then
...
sgs.ai_skill_exchange["jiebuqu"] = function(self, pattern, max_num, min_num, expand_pile)
    ...
    local hc = sgs.QList2Table(hcards)
    self:sortByKeepValue(hc, true)
    for _, c in pairs(hc) do
        local suit = c:getSuitString()
...
--法正
sgs.ai_skill_use["@@luadingjungive"] = function(self, prompt)
    local room = self.player:getRoom()
    local targets = room:getOtherPlayers(self.player)
    targets = room:sortByActionOrder(targets)
    local target2give
    local hcards = self.player:getCards("h")
    local toGive = {}
    local hasSlash, hasTrick = false, false
    for _, c in sgs.qlist(hcards) do
        if c:isKindOf("Slash") then
            hasSlash = true
        elseif c:getTypeId() == sgs.Card_TypeTrick and not (c:isKindOf("Lightning") or c:isKindOf("ImperialOrder")) then
            hasTrick = true
        end
        if hasSlash and hasTrick then
            break
        end
    end
    Global_room:writeToConsole("pass1")
    for _, p in sgs.qlist(targets) do
        if self.player:isFriendWith(p) and p:hasShownSkill("jizhi") and hasTrick then
            for _, c in sgs.qlist(hcards) do
                if c:getTypeId() == sgs.Card_TypeTrick and not (c:isKindOf("Lightning") or c:isKindOf("ImperialOrder")) then
                    table.insert(toGive, c:getEffectiveId())
                    target2give = p
                end
                if #toGive == 2 then
                    break
        ...
        elseif self.player:isFriendWith(p) and p:hasShownSkill("paoxiao|tieqi") and hasSlash then
            for _, c in sgs.qlist(hcards) do
                if c:isKindOf("Slash") then
                    table.insert(toGive, c:getEffectiveId())
                    target2give = p
            ...
            for _, p in sgs.qlist(targets) do
                if self.player:isFriendWith(p) and (self:willSkipPlayPhase() or self.player:getOverflow() > 1) then
                    hcards = sgs.QList2Table(hcards)
                    self:sortByUseValue(hcards, true)
                    for i = 1, 2 do
                        table.insert(toGive, hcards[i]:getEffectiveId())
                    end
                    target2give = p
                    break
                end
    ...
    Global_room:writeToConsole("pass3")
    if #toGive > 0 then
        return "#luadingjunCard:" .. table.concat(toGive, "+") .. ":&luadingjun->" .. target2give:objectName()
    end
    return ""
end
sgs.ai_skill_playerchosen.luadingjun = sgs.ai_skill_playerchosen.luaqinzheng
sgs.ai_skill_choice["luadingjun"] = function(self, choices, data)
    ...
    if self.player:isFriendWith(current) or self.player:willBeFriendWith(current) then
        if current:hasShownSkill("jizhi") and hasTrick and not self:willSkipPlayPhase(current) then
            for _, c in sgs.qlist(hcards) do
                if c:getTypeId() == sgs.Card_TypeTrick and not (c:isKindOf("Lightning") or c:isKindOf("ImperialOrder")) then
                    table.insert(toGive, c:getEffectiveId())
                end
            end
        elseif current:hasShownSkill("paoxiao") and hasSlash and not self:willSkipPlayPhase(current) then
            for _, c in sgs.qlist(hcards) do
                if c:isKindOf("Slash") then
                    table.insert(toGive, c:getEffectiveId())
                end
...
sgs.ai_skill_exchange["luadingjun"] = function(self, pattern, max_num, min_num, expand_pile)
    local num = min_num or 0
    if num < 2 then num = 2 end --定军至少要还给法正两张手牌
    local cards = sgs.QList2Table(self.player:getCards("h"))
    local current = self.room:getCurrent()
    if self.player:isFriendWith(current) then
        self:sortByUseValue(cards)
    else
        self:sortByKeepValue(cards) --保留价值最低的排在前面
    end
    local toGive = {}
    for _, c in ipairs(cards) do
        table.insert(toGive, c:getEffectiveId())
        if #toGive >= num then break end
    end
    return toGive
end

sgs.ai_skill_invoke.luaenyuan = sgs.ai_skill_invoke.enyuan
sgs.ai_skill_cardask["@luaenyuan-give"] = function(self, data)
    local hcards = self.player:getCards("h")
    local damage = data:toDamage()
    local to = damage.to
    local isfriend = self.player:isFriendWith(to)
    hcards = sgs.QList2Table(hcards)
    for _, c in pairs(hcards) do
        if c:getSuit() == sgs.suit_heart then
            if (c:isKindOf("Peach") or c:isKindOf("Analeptic")) and isfriend then
