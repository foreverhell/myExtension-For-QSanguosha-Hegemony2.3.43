...
jianmie = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        if player and player:isAlive() and player:hasSkill(self:objectName()) then  
            ...
            if damage.from and damage.from:objectName() == player:objectName() and not player:isFriendWith(damage.to) then  
...
caiwenji_hero = sgs.General(extension, "caiwenji_hero", "qun", 3, false)  

qiangshiObtain = sgs.CreateTriggerSkill{
    name = "qiangshiObtain",
    events = {sgs.CardFinished},
    can_trigger = function(self, event, room, player, data)
        local owner = room:findPlayerBySkillName(self:objectName())
        if not (owner and owner:isAlive() and owner:hasSkill(self:objectName())) then return "" end
        if owner:hasFlag("qiangshiObtain_first") then return "" end
        local current = room:getCurrent()
        local use = data:toCardUse()
        if use.from == owner or use.from ~= current then return "" end --其他角色，回合内
        if use.card:isNDTrick() then
            room:setPlayerFlag(owner, "qiangshiObtain_first")
            if not owner:isKongcheng() then
    ...
    on_cost = function(self, event, room, player, data, skill_owner)
        return skill_owner:askForSkillInvoke(self:objectName(), data) and room:askForDiscard(skill_owner, self:objectName(), 1, 1, true, false)
    end,
...
caiwenji_hero:addSkill(qiangshiObtain)
caiwenji_hero:addSkill("lirang")
sgs.LoadTranslationTable{  
    ["caiwenji_hero"] = "蔡文姬",
    ["qiangshiObtain"] = "强识",  
    [":qiangshiObtain"] = "其他角色回合内使用第一张非延时性锦囊结算完成后，你可以弃置一张手牌，获得之",  
}

caocao_hero = sgs.General(extension, "caocao_hero", "wei", 4)  
jianxiongCopyUse = sgs.CreateOneCardViewAsSkill{
    name = "jianxiongCopy",
    view_filter = function(self, to_select)
        return true
    end,
    ...
    view_as = function(self, cards)
        ...
        view_as_card:setShowSkill("jianxiongCopy")
        return view_as_card
	end,

    enabled_at_play = function(self, player)  
        local used_times = player:usedTimes("ViewAsSkill_jianxiongCopyCard")
        return not player:isNude() and used_times < 1 and player:hasFlag("turn_used")-- 出牌阶段可用  
    end  
}

jianxiongCopy = sgs.CreateTriggerSkill{
    name = "jianxiongCopy",
    events = {sgs.CardUsed, sgs.EventPhaseEnd},
    view_as_skill = jianxiongCopyUse,
    can_trigger = function(self, event, room, player, data)
...
caocao_hero:addSkill(jianxiongCopy)
sgs.LoadTranslationTable{  
    ["caocao_hero"] = "曹操",
    ["jianxiongCopy"] = "奸雄",
...
caogui = sgs.General(extension, "caogui", "wu", 3)--wu,qun
lunzhan = sgs.CreateTriggerSkill{  
    name = "lunzhan",  
    events = {sgs.CardUsed, sgs.EventPhaseEnd},  
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)
        if event == sgs.CardUsed then
            if player and player:isAlive() and player:hasSkill(self:objectName()) then
                ...
                local mark_name = "lunzhan_" .. tostring(use.card:getTypeId())
                if not player:hasFlag(mark_name) then
                    room:setPlayerFlag(player,mark_name)
                    return self:objectName()
                end
            end
        elseif event == sgs.EventPhaseEnd and player:getPhase() == sgs.Player_Finish then
            ...
            if owner and owner:isAlive() then
                room:setPlayerMark(owner,"@lunzhan_times",0)
            end
        end
        return ""
    end,  
    ...
    on_cost = function(self, event, room, player, data)  
        if player:hasShownSkill(self:objectName()) or player:askForSkillInvoke(self:objectName(), data) then
            room:addPlayerMark(player,"@lunzhan_times")
            return true  
        end  
        return false  
    end,  
    ...
    on_effect = function(self, event, room, player, data)
        local n = player:getMark("@lunzhan_times")
        player:drawCards(n,self:objectName())
        room:askForDiscard(player,self:objectName(),1,1,false,true)
    end
}
caogui:addSkill(lunzhan)
sgs.LoadTranslationTable{
    ["caogui"] = "曹刿",
    ["lunzhan"] = "论战",
    [":lunzhan"] = "你每回合首次使用一种类型的牌时，你摸X张牌并弃置1张牌。(X为本回合发动论战的次数)",
}
--[[
        ...
        if judge.card:isBlack() then  
            ...
            if target then 
                ...
                if cards:length() > 0 then  
                    ...
                    room:obtainCard(target, cards:first())
                end  
            end  
        end  
...
zhuding = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        ...
        elseif event == sgs.DrawNCards then
            ...
            data:setValue(num + math.min(player:getMark("@ding"),3))
        end  
        return ""  
    end,  
    on_cost = function(self, event, room, player, data)  
...
direnjie = sgs.General(extension, "direnjie", "qun", 3)  --qun, wu, shu  
...
qimou = sgs.CreateTriggerSkill{  
    ...
    on_cost = function(self, event, room, player, data, ask_who)  
        if ask_who:askForSkillInvoke(self:objectName(), data) then  
            room:setPlayerMark(ask_who,"@qimou",1)
    ...
    on_effect = function(self, event, room, player, data, ask_who)
        ...
        local choice = room:askForChoice(ask_who, self:objectName(), "top+bottom")  
        if choice == "top" then  
            room:moveCardTo(card, nil, sgs.Player_DrawPile, true)   
        else
...
shefu2 = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data, ask_who)  
        ...
        if card:getTypeId() == sgs.Card_TypeBasic then  
            if ask_who:isNude() or room:askForDiscard(ask_who, self:objectName(), 1, 1, false, true) then
                local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_SuitToBeDecided, -1)  
                slash:setSkillName(self:objectName())  
                if ask_who:canSlash(player, slash, false) then  
...
gaojianli = sgs.General(extension, "gaojianli", "wei", 3)  

zhujiVS = sgs.CreateViewAsSkill{  
    name = "zhuji",  
    n = 999, -- 可以选择所有手牌  
    view_filter = function(self, selected, to_select)  
...
zhuji = sgs.CreateTriggerSkill{  
    name = "zhuji",  
    events = {sgs.Damage},  
    view_as_skill = zhujiVS,  
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)  
        local damage = data:toDamage()  
        if damage.card and damage.card:getSkillName() == self:objectName() and damage.to and damage.to:isAlive() and not damage.to:isKongcheng() then  
            return self:objectName()  
        end  
        return ""  
    end,  
...
sgs.LoadTranslationTable{        
    ["gaojianli"] = "高渐离",  
    ["zhuji"] = "筑击",
    [":zhuji"] = "你可以将所有手牌当作杀使用或打出。若为使用，此杀造成伤害后，你可以弃置目标所有手牌。",--原版是伤害结算完成后
    ["lige"] = "离歌",
...
-- 养生主动技能  
yangshengVS = sgs.CreateViewAsSkill{  
    name = "yangsheng",  
    n = 1,  
    view_filter = function(self, selected, to_select)  
    ...
    on_effect = function(self, event, room, player, data, ask_who)
        if event == sgs.TurnStart and player:getMark("@tan") > 0 then
            ...
            if player:getHandcardNum() >= num then
                local choice = room:askForChoice(player, self:objectName(), "discard_" .. tostring(num) .. "+losehp")
...
hanshizhong = sgs.General(extension, "hanshizhong", "wu", 3)
xiejiaCard = sgs.CreateSkillCard{  
    name = "xiejiaCard",  
    target_fixed = true,  
    will_throw = true,  
    on_use = function(self, room, source, targets)
        source:drawCards(1,self:objectName())
        room:addPlayerMark(source,"@jia")
    end
}
xiejiaVS = sgs.CreateOneCardViewAsSkill{  
    name = "xiejia",
    filter_pattern = "EquipCard",  
    view_as = function(self, card)  
        local xiejiaCard = xiejiaCard:clone()
        xiejiaCard:addSubcard(card:getId())
        xiejiaCard:setSkillName(self:objectName())  
        xiejiaCard:setShowSkill(self:objectName())  
        return xiejiaCard  
    end,  
      
    enabled_at_play = function(self, player)  
        return not player:hasUsed("#xiejiaCard")  
    end  
}  
...
xiejia = sgs.CreateTriggerSkill{  
    name = "xiejia",  
    events = {sgs.DamageInflicted},
    view_as_skill = xiejiaVS,
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)  
        if player and player:isAlive() and player:hasSkill(self:objectName()) and player:getMark("@jia") > 0 then
            return self:objectName()
        end  
        return ""  
    end,  
    on_cost = function(self, event, room, player, data)  
        if player:askForSkillInvoke(self:objectName(), data) then
            room:broadcastSkillInvoke(self:objectName())
            room:setPlayerMark(player,"@jia",player:getMark("@jia")-1)
            return true  
        end  
        return false  
    end,  
...
hengjiangCard = sgs.CreateSkillCard{  
    name = "hengjiangCard",  
    target_fixed = false,  
    will_throw = true,  
    filter = function(self, targets, to_select)  
...
hengjiangVS = sgs.CreateZeroCardViewAsSkill{  
    name = "hengjiangVS",
    view_as = function(self)  
        local hengjiangCard = hengjiangCard:clone()
        hengjiangCard:setSkillName(self:objectName())  
        hengjiangCard:setShowSkill(self:objectName())  
        return hengjiangCard  
    end,  
      
    enabled_at_play = function(self, player)  
        return not player:hasUsed("#hengjiangCard")  
    end  
}  

hengjiangClear = sgs.CreateTriggerSkill{  
    name = "hengjiangVS",  
    events = {sgs.EventPhaseStart},
    view_as_skill = hengjiangVS,
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)  
        if player and player:isAlive() and player:hasSkill(self:objectName()) and player:getPhase()==sgs.Player_Start then
            for _,p in sgs.qlist(room:getAlivePlayers()) do
                room:setPlayerMark(p,"@jiang",0)
            end
        end  
        return ""  
    end,  
...
hengjiangMod = sgs.CreateDistanceSkill{
    name = "#hengjiangVS-distance",
    correct_func = function(self, from, to)
        if to:getMark("@jiang") > 0 then
			return 1
		end
		return 0
	end
}
hanshizhong:addSkill(xiejia)
hanshizhong:addSkill(hengjiangClear)
hanshizhong:addSkill(hengjiangMod)
extension:insertRelatedSkills("hengjiangVS","#hengjiangVS-distance")
sgs.LoadTranslationTable{
    ["hanshizhong"] = "韩世忠",
    ["xiejia"] = "卸甲",
    [":xiejia"] = "出牌阶段限一次。你可以弃置1张装备牌，摸1张牌并获得1个“甲”标记；你受到伤害时，你可以移除1个“甲”，令此次伤害无效",
    ["hengjiangVS"] = "横江",
    [":hengjiangVS"] = "出牌阶段限一次。你可以令一名角色获得【横江】标记，直到你的下回合，其他角色到其距离+1，然后若其装备区有装备，你可以获得其装备区1张装备，并令其摸1张牌"
}
hanxin = sgs.General(extension, "hanxin", "wu", 3)  -- 吴国，4血，男性  

gongxinFireVS = sgs.CreateZeroCardViewAsSkill{
    name = "gongxinFire",  
    view_as = function(self)  
...
gongxinFire = sgs.CreateTriggerSkill{  
    name = "gongxinFire",  
    events = {sgs.CardUsed},  
    view_as_skill = gongxinFireVS,
    can_trigger = function(self, event, room, player, data)  
        if not (player and player:isAlive() and player:hasSkill("gongxinFire")) then return "" end  
        -- 当使用衔镜技能时设置标记  
        local use = data:toCardUse()  
        card = use.card  
        if card:getSkillName() == "gongxinFire" then  
            return self:objectName()  
        end  
        return ""  
    end,  
    ...
    on_effect = function(self, event, room, player, data)  
        room:setPlayerFlag(player, "gongxinFire_used")  
        return false  
    end  
}
...
huamulan = sgs.General(extension, "huamulan", "wu", 4, false)  
huamulan:setHeadMaxHpAdjustedValue(-1)
-- 变装技能实现  
...
renzu = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data)
        if event == sgs.DrawNCards then
            local n = data:toInt()
            local count = room:getAlivePlayers():length() - 1
            data:setValue(n + math.min(count,3))
        elseif event == sgs.EventPhaseStart then
            if player:hasShownSkill(self:objectName()) and player:getPhase() == sgs.Player_Discard then  
                ...
                if handcards:length() > 0 then
                    ...
                    player:addToPile("zu", card_ids, false)
                    -- 可以让玩家选择顺序，这里简化为直接添加  
                    room:broadcastSkillInvoke(self:objectName(), player)  
                end  
            elseif player:getPhase() == sgs.Player_Start then  
...
huangyueying_hero = sgs.General(extension, "huangyueying_hero", "shu", 3, false)
zhifu = sgs.CreateTriggerSkill{  
    name = "zhifu",  
    events = {sgs.EventPhaseStart, sgs.EventPhaseEnd},  
    can_trigger = function(self, event, room, player, data)  
        if player and player:isAlive() and not player:hasSkill(self:objectName()) and player:getPhase() == sgs.Player_Play then
            if event == sgs.EventPhaseStart then
                local owner = room:findPlayerBySkillName(self:objectName())
                if owner and owner:isAlive() and owner:hasSkill(self:objectName()) then
                    return self:objectName(),owner:objectName()
                end
            elseif event == sgs.EventPhaseEnd and not player:isKongcheng() then
                local owner = room:findPlayerBySkillName(self:objectName())
                if owner and owner:isAlive() and owner:hasSkill(self:objectName()) and owner:hasFlag("zhifu_give") then
                    return self:objectName(),owner:objectName()
                end
            end
        end
    ...
    on_effect = function(self, event, room, player, data, ask_who)
        if event == sgs.EventPhaseStart then
            local trick = room:askForCard(ask_who, "TrickCard", "@zhifu-give:" .. player:objectName(), data, sgs.Card_MethodNone)  
            if trick then
                room:obtainCard(player,trick)
                room:setPlayerFlag(ask_who,"zhifu_give")
            end
        elseif event == sgs.EventPhaseEnd and not player:isKongcheng() then
            --[[
            local card_id = room:askForCardChosen(ask_who,player,"h",self:objectName(),true)
            if room:askForChoice(ask_who,self:objectName(),"yes+no") == "yes" then
                room:obtainCard(ask_who,card_id,false)
            end
            ]]
            local card_ids = room:askForCardsChosen(ask_who, player, "h", self:objectName(), 0, 1, true)  
            for _,card_id in sgs.qlist(card_ids) do
                room:obtainCard(ask_who,card_id,false)
            end
        end
    ...
    can_trigger = function(self, event, room, player, data)
        if player and player:isAlive() and player:getPhase() == sgs.Player_Finish then
            ...
            if owner and owner:isAlive() and owner:hasSkill(self:objectName()) then
                ...
                for _,card in sgs.qlist(owner:getHandcards()) do
...
huangyueying_hero:addSkill(zhifu)
huangyueying_hero:addSkill(huixin)
sgs.LoadTranslationTable{  
    ["huangyueying_hero"] = "黄月英",
    ["zhifu"] = "智辅",
    [":zhifu"] = "其他角色出牌阶段开始时，你可以交给其一张锦囊，若如此做，本阶段结束时，你观看其手牌并获得其至多1张牌",
    ["huixin"] = "慧心",
    [":huixin"] = "任意角色回合结束时，若你手牌中没有锦囊，你从摸牌堆随机获得1张锦囊",
}
huoqubing = sgs.General(extension, "huoqubing", "wu", 3) 
guanjunhou = sgs.CreateTriggerSkill{  
    name = "guanjunhou",  
    ...
    on_effect = function(self, event, room, player, data)
        if event == sgs.DrawNCards then
           local num = data:toInt()
           data:setValue(num+2)
           room:setPlayerFlag(player,"guanjunhou_used")
        elseif event == sgs.AfterDrawNCards then
...
jiufa = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data)  
        if event == sgs.CardFinished then  
            ...
            local target = room:askForPlayerChosen(player, targets, self:objectName(), "@jiufa-target", true)  
            if target then
                local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)  
                slash:setSkillName(self:objectName())  
                local use = sgs.CardUseStruct()  
...
jiangwei_hero:addSkill(jiufa)
jiangwei_hero:addSkill(weifu)
sgs.LoadTranslationTable{  
    ["jiangwei_hero"] = "姜维",
    ["jiufa"] = "九伐",
...
jingke = sgs.General(extension, "jingke", "wei", 4)  
...
cike = sgs.CreateTriggerSkill{
	...
	on_effect = function(self, event, room, skill_target, data, player)          
        ...
        elseif judge.card:isRed() then
            ...
            table.insert(disresponsive_list,skill_target:objectName())
...
sgs.LoadTranslationTable{
    ["jingke"] = "荆轲",
    ["cike"] = "刺客",
    [":cike"] = "你使用杀指定目标后，你可以发起一次判定：若判定牌为红色，该目标不可响应此杀；若判定牌为黑色，你弃置该目标1张牌"
}
...
mingchaCard = sgs.CreateSkillCard{  
    name = "mingcha",  
    target_fixed = false,  
    will_throw = true,  
...
shouliCard = sgs.CreateSkillCard{  
    name = "shouli",  
    target_fixed = false,  
    will_throw = false,  
...
chongruCard = sgs.CreateSkillCard{  
    name = "chongru",  
    target_fixed = false,  
    will_throw = false,  
...
yaoyue = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        ...
        local use = data:toCardUse()  
        -- 检查是否有角色使用了杀  
        if not (use.card and (use.card:isKindOf("Slash") or use.card:isNDTrick())) then return "" end  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        use.to:append(extra_target) --延时锦囊不能生效；但是目标要合法，不同锦囊不同，所以最好不要这样写
        data:setValue(use)
        return false  
    end  
}  
...
jiuxian = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        if player and player:isAlive() and player:hasSkill(self:objectName()) then
            if event == sgs.CardUsed then
                local use = data:toCardUse()  
                if use.card and use.card:isKindOf("Slash") and use.card:getNumber() >= 9 then  
                    return self:objectName()  
                end
...
lichunfeng = sgs.General(extension, "lichunfeng", "qun", 4)--wu,qun
...
feijiang_range = sgs.CreateAttackRangeSkill{  
    name = "#feijiang-range",  
    extra_func = function(self, player)  
        if player:hasShownSkill("feijiang") then  
            return 3
        end  
        return 0  
    end  
}  
liguang:addSkill(feijiang)
liguang:addSkill(feijiang_range)
sgs.LoadTranslationTable{
    ["liguang"] = "李广",
    ["feijiang"] = "飞将",
    [":feijiang"] = "你的攻击范围+3。你使用杀后，你可以发起一次判定：若判定牌为黑色，此杀不可响应；若判定牌为红色，你获得判定牌"
}
...
sgs.LoadTranslationTable{  
    ["liji"] = "骊姬",  
    ["dihui"] = "诋毁",  
    [":dihui"] = "锁定技。你使用杀对一名角色造成伤害后，其获得'追'标记，令其下次受到伤害时，伤害值+1，然后失去'追'标记。",  
    ["@zhui"] = "追",  
...
sgs.LoadTranslationTable{  
    ["linchong"] = "林冲",  
    ["baotou"] = "豹头",  
    [":baotou"] = "你使用杀不可闪避；且你可以令目标弃置手牌中所有闪",  
}
...
lixian = sgs.General(extension, "lixian", "shu", 4)  
weiyaCard = sgs.CreateSkillCard{  
    name = "weiyaCard",  
    target_fixed = false,--是否需要指定目标，默认false，即需要
    will_throw = false,
    filter = function(self, targets, to_select)  
    ...
    on_use = function(self, room, source, targets)  
        ...
        -- 询问目标是否对victim使用杀  
        local prompt = string.format("@weiya-slash:%s:%s:", victim:objectName(), target:objectName())  
        if room:askForUseSlashTo(target, victim, prompt, false, false, false) and victim == source then
            source:drawCards(1,self:objectName())
        else
            local card_ids = room:askForExchange(target, self:objectName(), 1, 1)
            for _,card_id in sgs.qlist(card_ids) do
...
weiya = sgs.CreateZeroCardViewAsSkill{
    name = "weiya",
    view_as = function(self)
        local xc = weiyaCard:clone()
        xc:setShowSkill(self:objectName())
        return xc
    end,

    enabled_at_play = function(self, player)
        return not player:hasUsed("#weiyaCard")
    end
}  

zhenfu = sgs.CreateTriggerSkill{
    name = "zhenfu",
    ...
    can_trigger = function(self, event, room, player, data)
        ...
        if event == sgs.Damage then
            room:setPlayerFlag(player,"zhenfu_damage")
        elseif event == sgs.DamageInflicted and player:hasFlag("zhenfu_damage") then
            return self:objectName()
        end
        return ""
    end,  
    ...
    on_cost = function(self, event, room, player, data)
        return player:hasShownSkill(self:findPlayerBySkillName()) or player:askForSkillInvoke(self:objectName(), data) 
    end,
...
lixian:addSkill(weiya)
lixian:addSkill(zhenfu)
sgs.LoadTranslationTable{  
    ["lixian"] = "李贤",
    ["weiya"] = "威压",
    [":weiya"] = "出牌阶段限一次。你可以令一名有牌的其他角色对你选择的其攻击范围内的另一名角色使用杀。若其使用杀且目标为自己，你摸1张牌；否则其交给你一张牌",
    ["zhenfu"] = "镇抚",
    [":zhenfu"] = "当你造成伤害后，本回合你免疫所有伤害",
}
...
luocheng = sgs.General(extension, "luocheng", "shu", 4)  
biaoqi = sgs.CreateTriggerSkill{
	name = "biaoqi",
	events = {sgs.CardUsed},
    frequency = sgs.Skill_Frequent,
	can_trigger = function(self, event, room, player, data)
		if skillTriggerable(player, self:objectName()) and event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card:isKindOf("Slash") and use.from == player and player:getAttackRange()>=2 then
				local target_list = {}
				for _, p in sgs.qlist(use.to) do
					if p ~= player then
						table.insert(target_list, p:objectName())
...
hanqiang = sgs.CreateAttackRangeSkill{  
    name = "hanqiang",  
    extra_func = function(self, player)  
        if player:hasShownSkill(self:objectName()) and not player:getWeapon() then  
            return 1
        end  
        return 0  
    end  
}  
luocheng:addSkill(biaoqi)
luocheng:addSkill(hanqiang)
sgs.LoadTranslationTable{
    ["luocheng"] = "罗成",
    ["biaoqi"] = "骠骑",
    [":biaoqi"] = "若你的攻击范围大于等于2，你使用杀时，若目标血量：大于等于3，你摸1张牌；小于等于2，该目标不可闪避此杀",
    ["hanqiang"] = "寒枪",
    [":hanqiang"] = "你没有装备武器时，你的攻击范围+1",
}
...
lvbu_hero = sgs.General(extension, "lvbu_hero", "qun", 4)  
fangtian = sgs.CreateTriggerSkill{
	...
	can_trigger = function(self, event, room, player, data)
		if skillTriggerable(player, self:objectName()) and event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card:isKindOf("Slash") and use.from == player and not player:isKongcheng() then
				local target_list = {}
				for _, p in sgs.qlist(use.to) do
	...
	on_effect = function(self, event, room, skill_target, data, player)
        if not player:isKongcheng() and not skill_target:isKongcheng() then
            if player:pindian(skill_target, self:objectName()) then
                local use = data:toCardUse()
                disresponsive_list = use.disresponsive_list
                table.insert(disresponsive_list,skill_target:objectName())
                use.disresponsive_list = disresponsive_list
...
shenji = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        local targets = sgs.SPlayerList()
        for _,p in sgs.qlist(room:getOtherPlayers(damage.to)) do
            if p ~= player and not p:hasFlag("shenji_target") and damage.to:distanceTo(p) <= 1 then
                targets:append(p)
            end
        end
        local target = room:askForPlayerChosen(player, targets, self:objectName(), self:objectName(), true) 
        if target then
            room:setPlayerFlag(target,"shenji_target")
            local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)  
            slash:setSkillName(self:objectName())  
            local use = sgs.CardUseStruct()  
            use.card = slash  
...
tianbian = sgs.CreateTriggerSkill{
    ...
    can_trigger = function(self, event, room, player, data)
        if skillTriggerable(player, self:objectName()) and event == sgs.PindianVerifying then
            ...
            if pd.from == player and pd.from_card:getSuit() == sgs.Card_Heart then
                return self:objectName()
            elseif pd.to == player and pd.to_card:getSuit() == sgs.Card_Heart then
                return self:objectName()
            end
        end
        return false
...
zijian = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        if event == sgs.EventPhaseEnd and player:getPhase() == sgs.Player_Finish then
            ...
            if not (zijian_player and zijian_player:isAlive() and zijian_player:hasSkill(self:objectName())) then return "" end
            -- 其他角色回合结束时
            if zijian_player ~= player and zijian_player:getCardCount(true) > 0 then
                return self:objectName(), zijian_player:objectName()
            end
        elseif event == sgs.DrawNCards and player:getMark("@zijian_draw") > 0 then
            local n = data:toInt()
            data:setValue(n + math.min(player:getMark("@zijian_draw"),3))
...
maowenlong = sgs.General(extension, "maowenlong", "qun", 3)--wei,qun
xuyi = sgs.CreateTriggerSkill{  
    name = "xuyi",  
    ...
    can_trigger = function(self, event, room, player, data)
        ...
        if event == sgs.CardFinished then
            ...
            if use.card:getTypeId() == sgs.Card_TypeSkill then return "" end
            local current = room:getCurrent()
            if use.from ~= current or use.from == owner then return "" end
    ...
    on_cost = function(self, event, room, player, data, ask_who)  
        if ask_who:askForSkillInvoke(self:objectName(), data) then
            room:setPlayerFlag(ask_who,"xuyi_used")
            return true  
        end  
        return false  
    end,  
    ...
    on_effect = function(self, event, room, player, data, ask_who)
        ...
        local card = sgs.Sanguosha:getCard(card_id)
        room:obtainCard(player,card,true)
        room:setPlayerMark(ask_who,"xuyi_card_id",card_id)
    end
}
maogong = sgs.CreateTriggerSkill{  
    name = "maogong",  
...
maowenlong:addSkill(xuyi)
maowenlong:addSkill(maogong)
sgs.LoadTranslationTable{
    ["maowenlong"] = "毛文龙",
    ["xuyi"] = "虚迤",
    [":xuyi"] = "其他角色在其回合内首次使用杀后，你可以查看牌堆顶1张牌并将其交给此角色；若该角色获得此牌并在本回合内使用，在其结算后你获得之。否则回合结束时其流失1点血量",--非装备非延时性锦囊
    ["maogong"] = "冒功",
    [":maogong"] = "任意角色失去血量时，你摸1张牌",
}
-- 创建武将：唐伯虎  
...
sgs.LoadTranslationTable{
    ...
    ["manwang"] = "蛮王",  
    [":manwang"] = "出牌阶段，你可以将两张杀视为【南蛮入侵】。",  
    ["qizong"] = "七纵",
    [":qizong"] = "你体力值变化时，获得摸牌堆的一张杀",

    ["manhou"] = "蛮后",   
    [":manhou"] = "出牌阶段，你可以将一张杀视为【决斗】。",
...
haoling = sgs.CreateTriggerSkill{
	...
	on_effect = function(self, event, room, skill_target, data, player)
		...
		if not target_inRange:isEmpty() then
			local target_askSlash = room:askForPlayerChosen(player, target_inRange, self:objectName(), "@JieAskto", true)
...
yaoji = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        --[[
        local chosen_players = room:askForPlayersChosen(player, room:getAlivePlayers(), self:objectName(), 2, 2, "请选择2名玩家，先选的视为对后选的使用杀", true)
...
lieboEndPlayPhase = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        if player and player:isAlive() and player:hasSkill(self:objectName()) then  
            local current = room:getCurrent()  
            -- 检查当前是否有回合角色且处于出牌阶段  
            if current and current:getPhase() == sgs.Player_Play and not player:isFriendWith(current) then  
                return self:objectName()  
            end  
        end  
        return ""  
...
panan = sgs.General(extension, "panan", "qun", 4)
...
qinliangyu = sgs.General(extension, "qinliangyu", "shu", 3, false)  

yinghuiVS = sgs.CreateOneCardViewAsSkill{  
    name = "yinghui",  
    filter_pattern = "TrickCard",  
    view_as = function(self, card)  
        local card_name = sgs.Self:getTag(self:objectName()):toString()
		if card_name ~= "" then
			local view_as_card = sgs.Sanguosha:cloneCard(card_name, card:getSuit(), card:getNumber())
			view_as_card:setCanRecast(false)
...
yinghui = sgs.CreateTriggerSkill{  
    name = "yinghui",  
    events = {sgs.CardUsed},  
    view_as_skill = yinghuiVS,
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)
		if skillTriggerable(player, self:objectName()) and event == sgs.CardUsed then
			local use = data:toCardUse()
...
pingpan = sgs.CreateTriggerSkill{  
    name = "pingpan",  
    ...
    can_trigger = function(self, event, room, player, data)
        if event == sgs.CardUsed then
            ...
            if use.card:isKindOf("SavageAssault") or use.card:isKindOf("ArcheryAttack") then
                ...
                local owner = room:findPlayerBySkillName(self:objectName())
                if not (owner and owner:isAlive() and owner:hasSkill(self:objectName())) then return "" end
                if not owner:willBeFriendWith(use.from) and not owner:hasFlag("pingpan_slash") then
                    return self:objectName(), owner:objectName()
                end
            end
        elseif event == sgs.SlashHit then
            local effect = data:toSlashEffect()
            if effect.from:hasFlag("pingpan_slash") then
                room:setPlayerFlag(effect.from,"pingpan_slash_hit")
            end
        elseif event == sgs.CardFinished then
            ...
            if use.card:isKindOf("SavageAssault") or use.card:isKindOf("ArcheryAttack") then
                local owner = room:findPlayerBySkillName(self:objectName())
                if not (owner and owner:isAlive() and owner:hasSkill(self:objectName())) then return "" end
                if not owner:willBeFriendWith(use.from) and owner:hasFlag("pingpan_slash") then
                    room:setPlayerFlag(owner,"-pingpan_slash")
                end
            end
        end
        return ""  
    ...
    on_effect = function(self, event, room, player, data, ask_who)
        local use = data:toCardUse()
        room:setPlayerFlag(ask_who,"pingpan_slash")
        room:askForUseSlashTo(ask_who, use.from, "你可以对该牌使用者使用杀，若该杀命中，该牌无效", false, false, false)
        if ask_who:hasFlag("pingpan_slash_hit") then --命中，该牌无效
            for _, p in sgs.qlist(use.to) do  
                use.to:removeOne(p)
            --    room:cancelTarget(use, p)
            --    sgs.Room_cancelTarget(use, p)
            ...
            data:setValue(use) --用以上方式修改use后，需要setValue  
            room:setPlayerFlag(ask_who,"-pingpan_slash_hit")
        end
        return false
    end  
}
qinliangyu:addSkill(yinghui)
qinliangyu:addSkill(pingpan)
sgs.LoadTranslationTable{  
    ["qinliangyu"] = "秦良玉",  
    ["yinghui"] = "颖慧",  
    [":yinghui"] = "你可以将锦囊当杀或闪使用或打出。若为使用杀，你可以获得目标一张牌",
    ["pingpan"] = "平叛",
    [":pingpan"] = "其他势力角色使用【南蛮入侵】【万箭齐发】时，你可以对其使用杀，若该杀命中，该锦囊无效",
}
...
nvxiang = sgs.CreateTriggerSkill{  
    name = "nvxiang",  
    events = {sgs.CardUsed}, --TargetConfirming
...
sgs.LoadTranslationTable{  
    ...
    ["nvxiang"] = "女相",  
    [":nvxiang"] = "任意角色使用杀时，你可以与其拼点，若你赢，则该杀无效。",  
...
wudaoDeath = sgs.CreateTriggerSkill{  
    name = "wudaoDeath",  
    events = {sgs.Death},
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)  
        if not (player and player:isAlive() and player:hasSkill(self:objectName())) then return "" end
        local death = data:toDeath()  
        if death.damage and death.damage.from:hasSkill(self:objectName()) and death.damage.from:getHandcardNum()<5 then
            return self:objectName()
        end
    end,  
    on_cost = function(self, event, room, player, data)  
    ...
    on_effect = function(self, event, room, player, data) 
        local count = 5-player:getHandcardNum()
...
shenwansan = sgs.General(extension, "shenwansan", "wei", 3)
jvbao = sgs.CreateTriggerSkill{  
    name = "jvbao",  
    events = {sgs.TurnStart, sgs.DrawNCards},
    frequency = sgs.Skill_Compulsory,
    can_trigger = function(self, event, room, player, data)  
        if event == sgs.TurnStart and not player:hasSkill(self:objectName()) then
            local owner = room:findPlayerBySkillName(self:objectName())
            if not (owner and owner:isAlive() and owner:hasSkill(self:objectName())) then return "" end
            -- 其他角色回合结束时
            if owner:getMark("@bao") < 5 then
                room:addPlayerMark(owner,"@bao")
            end
        elseif event == sgs.DrawNCards and player:getMark("@bao") >= 5 then
            return self:objectName()
        end  
        return ""  
    end,  
    ...
    on_effect = function(self, event, room, player, data)  
        local n = data:toInt()
        data:setValue(n + 2)
        room:removePlayerMark(player,"@bao",1)
        return false  
    end  
}
sancai = sgs.CreateTriggerSkill{  
    name = "sancai",  
    events = {sgs.Dying},
    frequency = sgs.Skill_Compulsory,
    can_trigger = function(self, event, room, player, data)  
        if not (player and player:isAlive() and player:hasSkill(self:objectName())) then return "" end
        local dying = data:toDying()
        if player == dying.who and player:getMark("@bao") > 0 then
            return self:objectName()
        end
        return ""
    end,  
    ...
    on_effect = function(self, event, room, player, data)
        local count = player:getMark("@bao")
        local lose_count = 0
        if count >= 3 then
            room:setPlayerMark(player,"@bao",0)
            lose_count = count
            local recover = sgs.RecoverStruct()  
            recover.who = player  
        ...
        if lose_count > 0 then
            player:drawCards(lose_count,self:objectName())
            local dying = data:toDying()
            if dying.damage and dying.damage.from then
                dying.damage.from:drawCards(lose_count,self:objectName())
            end
        end
    end
}
shenwansan:addSkill(jvbao)
shenwansan:addSkill(sancai)
sgs.LoadTranslationTable{
    ["shenwansan"] = "沈万三",  

    ["jvbao"] = "聚宝",   
    [":jvbao"] = "锁定技。其他角色回合开始时，你获得1个“宝”标记；你的摸牌阶段，若“宝”大于等于5，你移除1个“宝”，令本回合摸牌数+2。“宝”至多为5",
    ["sancai"] = "散财",
    [":sancai"] = "锁定技。你进入濒死时，若你的“宝”大于等于3，你移除所有“宝”标记并将体力值回复到1，否则你移除1个“宝”标记。然后你与伤害源各摸X张牌，X为移除“宝”标记的数量"
}
-- 创建武将：时迁  
...
JunhengCard = sgs.CreateSkillCard{  
    ...
    filter = function(self, selected, to_select)  
        ...
        if #selected == 1 then return selected[1]:getHandcardNum() ~= to_select:getHandcardNum() end
        if #selected >= 2 then return false end
    end,  
    ...
    on_use = function(self, room, source, targets)  
        ...
        if targets[1]:getHandcardNum() < targets[2]:getHandcardNum() then
            from = targets[1]
            to = targets[2]
        elseif targets[1]:getHandcardNum() > targets[2]:getHandcardNum() then
            from = targets[2]
            to = targets[1]
        elseif targets[1]:getHandcardNum() == targets[2]:getHandcardNum() then
            return false
...
-- 创建均衡视为技  
Junheng = sgs.CreateZeroCardViewAsSkill{  
    name = "junheng",
    view_as = function(self)  
...
sgs.LoadTranslationTable{  
    ["sunquan_hero"] = "孙权",
    ["junheng"] = "均衡",  
    [":junheng"] = "出牌阶段限1次，你可以选择两名手牌数不相等的角色，令手牌数更少的角色获得手牌数更多的角色一张手牌，若此时他们手牌数相等，你摸一张牌。",  
    ["junhengCard"] = "均衡",  
    ["beixi"] = "背袭",  
...
yuzhu = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data, ask_who)
        local success = ask_who:pindian(player, "yuzhu")
        if success then
            local source = room:askForPlayerChosen(ask_who, room:getOtherPlayers(player), self:objectName())
            local slash = sgs.Sanguosha:cloneCard("slash")  
            slash:setSkillName(self:objectName())  
            local use = sgs.CardUseStruct()  
            use.from = source  
...
rangquan = sgs.CreateTriggerSkill{  
    name = "rangquan",  
    events = {sgs.Damaged},  
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)  
...
taipinggongzhu:addSkill(yuzhu)
taipinggongzhu:addSkill(rangquan)
sgs.LoadTranslationTable{
    ["taipinggongzhu"] = "太平公主",
    ["yuzhu"] = "预诛",
...
saimaCard = sgs.CreateSkillCard{  
    ...
    filter = function(self, targets, to_select)  
        -- 第一个目标是接收手牌并使用杀的角色  
        if #targets == 0 and to_select:objectName() ~= sgs.Self:objectName() and not to_select:isKongcheng() then
            if not sgs.Self:hasUsed("#saimaCard") then return true end
            return to_select:hasFlag("saima_target")
        end
        return false
    end,
    on_use = function(self, room, source, targets)  
...
saima = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data)
        if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Finish then
            ...
            damage.from = winner  
            damage.to = loser  
            damage.damage = 1--winner:getMark("@saima_win")
            damage.reason = self:objectName()  
            room:damage(damage)  
            winner:drawCards(winner:getMark("@saima_win")-1,self:objectName())
...
sgs.LoadTranslationTable{
    ["tianji4"] = "田忌",
    ["saima"] = "赛马",
    [":saima"] = "出牌阶段你可以与一名其他角色拼点至多3次，结束阶段，获胜更多者执行：另一方造成1点伤害，摸X-1张牌（X为其获胜次数）。",
    ["weijiu"] = "围救",
    [":weijiu"] = "出牌阶段限1次，你可以摸1张牌并交给其他角色1张手牌，视为你对其使用1张不计入次数的杀"
}

shesaihua = sgs.General(extension, "shesaihua", "shu", 4, false)

guashuai = sgs.CreateTriggerSkill{  
    name = "guashuai",  
    events = {sgs.EventPhaseStart},  
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)  
        if player and player:isAlive() and player:hasSkill(self:objectName()) then
            ...
            elseif player:getPhase() == sgs.Player_Finish then
                room:setPlayerMark(player,"@guashuai_distance",0)
            end
        end  
        return ""  
    end,  
    ...
    on_effect = function(self, event, room, player, data)  
        for _, p in sgs.qlist(room:getOtherPlayers(player)) do
            local card = room:askForCard(p, "Slash", "@guashuai-give", data, sgs.Card_MethodNone) --".|.|.|hand,equipped"
            if card then
                room:obtainCard(player, card:getId())
                room:addPlayerMark(player,"@guashuai_distance")
            end
        end
        return false  
    end  
}
guashuaiMod = sgs.CreateDistanceSkill{
    name = "#guashuai-distance",
    correct_func = function(self, from, to)
		if from:hasShownSkill(self:objectName()) then
			return -from:getMark("@guashuai_distance")
		end
		return 0
	end
}
...
zhaoqinCard = sgs.CreateSkillCard{  
    name = "zhaoqinCard",  
    target_fixed = false,  
    will_throw = false,  
    filter = function(self, targets, to_select)  
        return to_select:hasShownOneGeneral() and to_select:isMale() and sgs.Self:distanceTo(to_select) == 1 and not to_select:isKongcheng()
    end,  
    feasible = function(self, targets)  
        return #targets > 0
    end,  
    on_use = function(self, room, source, targets)
        ...
        for _, p in ipairs(targets) do
            ...
            shown[p:objectName()] = card:getSuit()
        end
        --自己展示手牌
        local card = room:askForCardShow(source, source, self:objectName())
        room:showCard(source, card:getId())
        --依次判断花色是否相同
        for _, p in ipairs(targets) do
            if shown[p:objectName()] == card:getSuit() then
                source:drawCards(1,self:objectName())
...
zhaoqin = sgs.CreateZeroCardViewAsSkill{  
    name = "zhaoqin",  
    view_as = function(self)  
        local zhaoqinCard = zhaoqinCard:clone()  
        zhaoqinCard:setSkillName(self:objectName())  
        zhaoqinCard:setShowSkill(self:objectName())  
        return zhaoqinCard  
    end,  
      
    enabled_at_play = function(self, player)  
        return not player:isKongcheng() and not player:hasUsed("#zhaoqinCard")  
    end  
}  
shesaihua:addSkill(guashuai)
shesaihua:addSkill(guashuaiMod)
shesaihua:addSkill(zhaoqin)
extension:insertRelatedSkills("guashuai", "#guashuai-distance")
sgs.LoadTranslationTable{
    ["shesaihua"] = "佘赛花",
    ["guashuai"] = "挂帅",
    [":guashuai"] = "你的出牌阶段开始时，其他角色可以交给你一张杀，并令你本回合到其他角色的距离-1",
    ["zhaoqin"] = "招亲",
    [":zhaoqin"] = "出牌阶段限一次。你可以令与你距离为1的任意名其他男性角色依次展示1张手牌，随后你展示一张手牌，若其展示的手牌花色：与你相同，你与其各摸1张牌；与你不同，你视为对其使用1张不计入次数的杀"
}
...
wangjian = sgs.General(extension, "wangjian", "qun", 4)

hengsaoVS = sgs.CreateViewAsSkill{  
    name = "hengsao",  
    n = 999, -- 可以选择所有手牌  
    view_filter = function(self, selected, to_select)  
        return #selected < sgs.Self:getHandcardNum() and not to_select:isEquipped() 
    ...
    view_as = function(self, cards)  
        ...
        archery_attack:setShowSkill(self:objectName())  
        for _, card in ipairs(cards) do  
    ...
    enabled_at_play = function(self, player)  
        return not player:isKongcheng() and not player:hasFlag("hengsao_used")
    end,  
}  

hengsao = sgs.CreateTriggerSkill{  
    name = "hengsao",  
    events = {sgs.CardUsed, sgs.Damage, sgs.CardFinished},  
    view_as_skill = hengsaoVS,
    can_trigger = function(self, event, room, player, data)  
        if not (player and player:isAlive() and player:hasSkill(self:objectName())) then return "" end
        if event == sgs.CardUsed then
            local use = data:toCardUse()
            if use.card:getSkillName() == self:objectName() then  
                room:setPlayerFlag(player, "hengsao_used") 
            end
        elseif event == sgs.Damage then
            local damage = data:toDamage()
            if damage.card:getSkillName() == self:objectName() then
                room:setPlayerFlag(player, "hengsao_damage") 
            end
        elseif event == sgs.CardFinished then
            local use = data:toCardUse()
            if use.card:getSkillName() == self:objectName() and not player:hasFlag("hengsao_damage") then
                player:drawCards(use.card:getSubcards():length(),self:objectName())
            end
        end
        return ""  
    end,  
...
qingshangCard = sgs.CreateSkillCard{  
    name = "qingshangCard",  
    skill_name = "qingshang",  
    target_fixed = false,  
    will_throw = true,
    filter = function(self, targets, to_select)  
        return #targets == 0
    ...
    on_use = function(self, room, source, targets)  
        ...
        target:drawCards(1, self:objectName())
        if target ~= source then
            local card_ids = room:askForExchange(target, self:objectName(), 1, 1, "@qingshang-exchange", "", ".|.|.|hand")
            for _,card_id in sgs.qlist(card_ids) do
                room:obtainCard(source, card_id, false)  
            end
        end
...
qingshangVS = sgs.CreateZeroCardViewAsSkill{  
    name = "qingshang",  
    view_as = function(self)  
        local card = qingshangCard:clone()
        card:setShowSkill(self:objectName())
        return card
    end,  
    enabled_at_play = function(self, player)  
        return not player:hasUsed("#qingshangCard") and player:hasFlag("qingshang_damage")
    end,  
}  

qingshang = sgs.CreateTriggerSkill{  
    name = "qingshang",  
    events = {sgs.Damage},  
    view_as_skill = qingshangVS,
    can_trigger = function(self, event, room, player, data)  
        if not (player and player:isAlive() and player:hasSkill(self:objectName())) then return "" end
        if event == sgs.Damage then
            room:setPlayerFlag(player, "qingshang_damage") 
        end
        return ""  
    end,  
...
wangjian:addSkill(hengsao)
wangjian:addSkill(qingshang)
sgs.LoadTranslationTable{  
    ["wangjian"] = "王翦",  
    ["hengsao"] = "横扫",
...
weiqing = sgs.General(extension, "weiqing", "wu", 4)  
huguo = sgs.CreateTriggerSkill{
	name = "huguo",
    ...
    can_trigger = function(self, event, room, player, data)
        ...
        elseif event == sgs.CardUsed then
            local use = data:toCardUse()
            if use.card:isKindOf("Slash") and use.from:hasFlag("huguo_hit") then
                use.card:setFlags("GlobalCardUseDisresponsive")  -- 设置不可响应标志  
            end
        end
        return ""
    ...
    on_effect = function(self, event, room, player, data)
        local isMinHandcardNum = true
        for _,p in sgs.qlist(room:getOtherPlayers(player)) do
            if player:getHandcardNum() > p:getHandcardNum() then
                isMinHandcardNum = false
                break
            end
        end
        if isMinHandcardNum then
            local n = data:toInt()
            data:setValue(n+1)
            room:setPlayerFlag(player, "huguo_hit")
        else
            room:setPlayerFlag(player, "huguo_extra")
        end
		return false
	end,
}
huguoMod = sgs.CreateTargetModSkill{  
    name = "#huguo-mod",  
    pattern = "Slash",  
    residue_func = function(self, player, card)  
        if player:hasFlag("huguo_extra") then  
            return 1  
        else  
            return 0  
        end  
...
qiantui = sgs.CreateTriggerSkill{  
    name = "qiantui",  
    events = {sgs.EventPhaseChanging},  
    frequency = sgs.Skill_Frequent,
    ...
    can_trigger = function(self, event, room, player, data)  
        ...
        local change = data:toPhaseChange()  
        if change.to == sgs.Player_Discard and not player:isKongcheng() then 
            return self:objectName()
        end  
        return ""  
    end,  
    ...
    on_cost = function(self, event, room, player, data)  
        return player:askForSkillInvoke(self:objectName(), data) and room:askForDiscard(player, self:objectName(), 1, 1, true, false) 
    end,  
...
weiqing:addSkill(huguo)
weiqing:addSkill(huguoMod)
weiqing:addSkill(qiantui)

sgs.LoadTranslationTable{  
    ["weiqing"] = "卫青",  
    ["huguo"] = "护国",  
    [":huguo"] = "摸牌阶段，若你的手牌数全场最少，你摸牌数+1且本回合杀不可闪避，否则你本回合使用杀次数+1",
    ["qiantui"] = "谦退",  
    [":qiantui"] = "弃牌阶段开始时，你可以弃置1张手牌，令1名势力相同的角色摸一张牌",  
}
weizheng = sgs.General(extension, "weizheng", "wu", 4)  -- 吴国，4血，男性  

jingjianCard = sgs.CreateSkillCard{  
    name = "jingjianCard",  
    target_fixed = false,  
    will_throw = false,  
...
jingjian = sgs.CreateZeroCardViewAsSkill{  
    ...
    view_as = function(self)  
        local card = jingjianCard:clone()
        card:setSkillName("jingjian")  
        return card  
    end,  
        
    enabled_at_play = function(self, player)  
        return not player:hasUsed("#jingjianCard")
    end  
}  
...
nvhuang = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        if player:getPhase() == sgs.Player_Start then    
            ...
            if wuzetian and wuzetian:isAlive() 
            and not wuzetian:isKongcheng() and not player:isKongcheng() 
            and not wuzetian:willBeFriendWith(player) then  
...
sgs.LoadTranslationTable{  
    ...
    ["nvhuang"] = "女皇",  
    [":nvhuang"] = "其他势力角色的准备阶段，你可以与其拼点，若你赢，其本回合不能使用锦囊牌。",  
...
xunyu_hero = sgs.General(extension, "xunyu_hero", "wei", 3)  
quhuSlashCard = sgs.CreateSkillCard{  
    name = "quhuSlashCard",  
    target_fixed = true,--是否需要指定目标，默认false，即需要
    will_throw = false,
    ...
    on_use = function(self, room, source, targets)
        --最大手牌，最大HP
        local max_handcardnum = source:getHandcardNum()
        local max_hp = source:getHp()
        for _, p in sgs.qlist(room:getAlivePlayers()) do
            if p:getHandcardNum() > max_handcardnum then max_handcardnum = p:getHandcardNum() end
            if p:getHp() > max_hp then max_hp = p:getHp() end
        end
        --选择最大手牌角色
        local max_handcardnum_targets = sgs.SPlayerList()
        for _, p in sgs.qlist(room:getAlivePlayers()) do
            if p:getHandcardNum() == max_handcardnum then max_handcardnum_targets:append(p) end
        end 
        local target1 = room:askForPlayerChosen(source, max_handcardnum_targets, "选择一名手牌数最多的角色，令其使用杀")

        local max_hp_targets = sgs.SPlayerList()
        for _, p in sgs.qlist(room:getOtherPlayers(target1)) do
            if p:getHp() == max_hp then max_hp_targets:append(p) end
        end 
        local target2 = room:askForPlayerChosen(source, max_hp_targets, "选择一名生命最高的角色")
        if not (target1 and target2) then return false end
        -- 询问目标是否对victim使用杀  
        local prompt = string.format("@quhuSlash-slash:%s:%s:", target2:objectName(), target1:objectName())  
        if not room:askForUseSlashTo(target1, target2, prompt, false, false, false) then  
            local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)  
            slash:setSkillName(self:objectName())  
            local use = sgs.CardUseStruct()  
            use.card = slash  
...
quhuSlash = sgs.CreateZeroCardViewAsSkill{
    name = "quhuSlash",
    view_as = function(self)
        local xc = quhuSlashCard:clone()
        xc:setShowSkill(self:objectName())
        return xc
    end,

    enabled_at_play = function(self, player)
        return not player:hasUsed("#quhuSlashCard")
    end
}

wangzuoCard = sgs.CreateSkillCard{  
    name = "wangzuoCard",  
    target_fixed = false,  
    will_throw = true,  
    filter = function(self, targets, to_select)  
    ...
    on_use = function(self, room, source, targets)
        local target = targets[1]
        local choices = {}
        if not source:hasFlag("wangzuoRecover") then table.insert(choices,"wangzuoRecover") end
        if not source:hasFlag("wangzuoLose") then table.insert(choices,"wangzuoLose") end
        local choice = room:askForChoice(source,self:objectName(),table.concat(choices, "+"))
        if choice == "wangzuoRecover" then
            local recover = sgs.RecoverStruct()  
            recover.who = target  
            recover.recover = 1  
            room:recover(target, recover)
            room:askForDiscard(target,self:objectName(),2,2,false,true)
        elseif choice == "wangzuoLose" then
            room:loseHp(target,1)
            target:drawCards(2,self:objectName())
        end
...
wangzuo = sgs.CreateZeroCardViewAsSkill{
    name = "wangzuo",
    view_as = function(self)
        local xc = wangzuoCard:clone()
        xc:setShowSkill(self:objectName())
        return xc
    end,

    enabled_at_play = function(self, player)
        return not player:hasFlag("wangzuoRecover") or not player:hasFlag("wangzuoLose")
    end
}
xunyu_hero:addSkill(quhuSlash)
xunyu_hero:addSkill(wangzuo)
sgs.LoadTranslationTable{  
    ["xunyu_hero"] = "荀彧",
    ["quhuSlash"] = "驱虎",
    [":quhuSlash"] = "出牌阶段限一次。你可以令手牌最多的角色对体力值最多的其他角色使用杀，若其未使用杀，则视为被对方杀",
    ["wangzuo"] = "王佐",
    [":wangzuo"] = "出牌阶段各限一次。你可以令一名角色恢复/失去1点体力并弃置/摸2张牌",
    ["wangzuoRecover"] = "恢复1点体力，并弃置2张牌",
    ["wangzuoLose"] = "失去1点体力，并摸2张牌",
}
...
miaosuan = sgs.CreateTriggerSkill{  
    name = "miaosuan",  
    ...
    can_trigger = function(self, event, room, player, data)  
        ...
        if player:hasSkill(self:objectName()) then --自己
            if event==sgs.EventPhaseStart and player:getPhase() == sgs.Player_Finish then --自己的结束阶段恢复次数
                room:setPlayerMark(player,"@miaosuan",1)
            end
        else --不是自己
            local owner = room:findPlayerBySkillName(self:objectName())--找到自己
            if not (owner and owner:isAlive() and owner:hasSkill(self:objectName())) then 
                return ""
            end
            if event == sgs.EventPhaseStart then
                ...
                elseif player:getPhase() == sgs.Player_Finish and player:hasFlag("miaosuan_target") then --目标结束阶段
                    ...
                    if not owner:hasFlag("miaosuan_draw") then --没有因此摸牌
                        player:turnOver() --叠置
                    end
                end
            elseif event == sgs.CardUsed then
                local use = data:toCardUse()
                if use.from:hasFlag("miaosuan_target") then
                    local card = use.card
                    local draw_num = 0
                    if card:getTypeId() == owner:getMark("@miaosuan-type") and not owner:hasFlag("miaosuan_draw_type") then
                        draw_num = draw_num + 1
                        room:setPlayerFlag(owner, "miaosuan_draw_type")
                    end
                    if card:getSuit() == owner:getMark("@miaosuan-suit") and not owner:hasFlag("miaosuan_draw_suit") then
                        draw_num = draw_num + 1
                        room:setPlayerFlag(owner, "miaosuan_draw_suit")
                    end
                    if card:getNumber() == owner:getMark("@miaosuan-number") and not owner:hasFlag("miaosuan_draw_number") then
                        draw_num = draw_num + 1
                        room:setPlayerFlag(owner, "miaosuan_draw_number")
                    end
                    if draw_num > 0 then
                        owner:drawCards(draw_num,self:objectName())
                        room:setPlayerFlag(owner, "miaosuan_draw")
                    end
                end
            end
        end
    ...
    on_cost = function(self, event, room, player, data, ask_who)  
        -- 询问是否发动技能  
        if room:askForSkillInvoke(ask_who, self:objectName(), data) then
            local card = room:askForCard(ask_who,".","@miaosuan-discard",sgs.QVariant(),sgs.Card_MethodDiscard) --弃牌
            if card then
                room:setPlayerFlag(player, "miaosuan_target") --标记对谁使用了这个技能
                room:setPlayerMark(ask_who,"@miaosuan",0) --标记本轮已使用
                room:setPlayerMark(ask_who,"@miaosuan-type",card:getTypeId())
                room:setPlayerMark(ask_who,"@miaosuan-suit",card:getSuit())
                room:setPlayerMark(ask_who,"@miaosuan-number",card:getNumber())
            end
            return false  
        end  
        return false  
...
tianxiang4 = sgs.CreateTriggerSkill{  
    name = "tianxiang4",  
    events = {sgs.EventPhaseStart},  
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        local max_num = player:getHandcardNum()
...
yuanshoucheng:addSkill(miaosuan)
yuanshoucheng:addSkill(tianxiang4)
sgs.LoadTranslationTable{
    ["yuanshoucheng"] = "袁守诚",
    ["miaosuan"] = "妙算",
    [":miaosuan"] = "每轮限一次。其他角色出牌阶段开始时，你可以弃置1张牌，其本回合首次使用相同类型/相同花色/相同点数的牌时，你摸一张牌；其本回合结束时，若你本回合未因此摸牌，其叠置",
    ["tianxiang4"] = "天相",
...
yuenv = sgs.General(extension, "yuenv", "wu", 4, false)--shu,wu
shoujianCard = sgs.CreateSkillCard{  
    name = "shoujianCard",  
    target_fixed = false,  
    will_throw = true,  
    filter = function(self, targets, to_select)  
    ...
    on_use = function(self, room, source, targets)
        ...
        if not slash_ids:isEmpty() then  
            ...
            local card = sgs.Sanguosha:getCard(id)  
            room:obtainCard(target, card, true)
            room:addPlayerMark(target,"@shoujian_extra_slash")
        end  
    end
}
shoujian = sgs.CreateOneCardViewAsSkill{  
    name = "shoujian",
    filter_pattern = "EquipCard",  
    view_as = function(self, card)  
        local shoujianCard = shoujianCard:clone()
        shoujianCard:addSubcard(card:getId())
        shoujianCard:setSkillName(self:objectName())  
        shoujianCard:setShowSkill(self:objectName())  
        return shoujianCard  
    end,  
      
    enabled_at_play = function(self, player)  
        return not player:hasUsed("#shoujianCard")  
    end  
}  

shoujianMod = sgs.CreateTargetModSkill{  
    name = "#shoujian-mod",
    pattern = "Slash",  
    residue_func = function(self, player, card)
        return player:getMark("@shoujian_extra_slash")
    end  
}
...
yuenv:addSkill(shoujian)
yuenv:addSkill(shoujianMod)
yuenv:addSkill(jiandao)
extension:insertRelatedSkills("shoujian", "#shoujian-mod")
sgs.LoadTranslationTable{
    ["yuenv"] = "越女",
    ["shoujian"] = "授剑",
...
zhangfei_hero = sgs.General(extension, "zhangfei_hero", "shu", 4)
shenyongVS = sgs.CreateOneCardViewAsSkill{
    name = "shenyong",
    view_filter = function(self, to_select)
        local suit = sgs.Self:property("shenyongSuit"):toInt()
        return not to_select:isEquipped() and to_select:getSuit() == suit
    end,

    view_as = function(self, card)
        local view_as_card = sgs.Sanguosha:cloneCard("duel",card:getSuit(),card:getNumber())
        view_as_card:setCanRecast(false)
        view_as_card:addSubcard(card:getId())
        view_as_card:setSkillName(self:objectName())
        view_as_card:setShowSkill(self:objectName())
...
shenyong = sgs.CreateTriggerSkill{  
    name = "shenyong",  
    events = {sgs.EventPhaseStart},
    view_as_skill = shenyongVS,
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)  
        if player and player:isAlive() and player:hasSkill(self:objectName()) then
            if player:getPhase() == sgs.Player_Play then  
                return self:objectName()
            elseif player:getPhase() == sgs.Player_Finish then
                room:setPlayerProperty(player, "shenyongSuit", sgs.QVariant())
            end
        end  
        return ""  
    end,  
    ...
    on_effect = function(self, event, room, player, data)  
        local suit = room:askForSuit(player, self:objectName()) --这个直接就可以选花色，不需要再转换一次
        room:setPlayerProperty(player, "shenyongSuit", sgs.QVariant(suit))
        return false  
    end  
}

podi = sgs.CreateTriggerSkill{  
    name = "podi",  
    ...
    can_trigger = function(self, event, room, player, data)
        if event == sgs.Damage then
            local damage = data:toDamage()
            if damage.card and damage.card:isKindOf("Duel") and damage.from and damage.from:hasSkill(self:objectName()) then
                return self:objectName()
            end
        elseif event == sgs.SlashEffected then
            local effect = data:toSlashEffect()  
            if effect.to and effect.to:hasFlag("podi_damage") then  
                effect.jink_num = effect.jink_num + 1  
                data:setValue(effect)  
            end
        end
    ...
    on_effect = function(self, event, room, player, data)  
        local damage = data:toDamage()
        room:setPlayerFlag(damage.to, "podi_damage")
        return false  
    end  
}

zhangfei_hero:addSkill(shenyong)--或者直接用颜良文丑的技能？
zhangfei_hero:addSkill(podi)
sgs.LoadTranslationTable{
    ["zhangfei_hero"] = "张飞",
    ["shenyong"] = "神勇",
    [":shenyong"] = "出牌阶段开始时，你选择1个花色，本回合你该花色的手牌可当作决斗使用",
    ["podi"] = "破敌",
    [":podi"] = "锁定技。你对其他角色造成决斗伤害后，其本回合响应杀需要闪的数量+1"
}
...
tuwei = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data)
        ...
        if player:isKongcheng() or isMinHp then
            local num = data:toInt()
            data:setValue(num+1)
        end
        return false  
...
zhaokuo:addSkill(tuwei)
sgs.LoadTranslationTable{
    ["zhaokuo"] = "赵括",
    ["zhishangtanbing"] = "纸上谈兵",  
    [":zhishangtanbing"] = "锁定技。每回合限一次，当其他角色受到伤害时，你摸一张牌；每回合限一次，当你受到伤害时，你弃置一张牌",
    ["tuwei"] = "突围",  
    [":tuwei"] = "锁定技。摸牌阶段开始时，若你没有手牌或体力值最低，你摸牌数+1",
}

zhaoyong = sgs.General(extension, "zhaoyong", "shu", 3)--shu,wu
...
hufuMaxCards = sgs.CreateMaxCardsSkill{  
    name = "#hufu-maxCards",  
    extra_func = function(self, player)
        if player:hasShownSkill("hufu") and player:getArmor() then return 4 end
        return 0
    end  
}
hanbei = sgs.CreateTriggerSkill{
    ...
    can_trigger = function(self, event, room, player, data)
		if skillTriggerable(player, self:objectName()) and event == sgs.CardUsed then
			local use = data:toCardUse()
			if use.card:isKindOf("Slash") and use.from == player and player:getWeapon() then
				return self:objectName()
			end
		end
		return false
...
zhaoyong:addSkill(hufu)
zhaoyong:addSkill(hufuMaxCards)
zhaoyong:addSkill(hanbei)
sgs.LoadTranslationTable{
    ["zhaoyong"] = "赵雍",
    ["hufu"] = "胡服",  
...
gaiguo = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        if player and player:isAlive() and player:hasSkill(self:objectName()) then  
            ...
            if player:getMark("@guo") % 3 == 0 then
                return self:objectName()
            end
        end  
        return ""  
...
sgs.LoadTranslationTable{  
    ...
    ["gaiguo"] = "改过",  
    [":gaiguo"] = "锁定技。你造成或受到1点伤害时，获得1个'过'标记。你的'过'标记为3的倍数时，摸三张牌。",  
...
zhudi = sgs.General(extension, "zhudi", "wu", 4)  
  
jingnan = sgs.CreateTriggerSkill{
	name = "jingnan",
	events = {sgs.CardUsed, sgs.EventPhaseEnd},
    frequency = sgs.Skill_Frequent,
	can_trigger = function(self, event, room, player, data)
	...
	on_effect = function(self, event, room, skill_target, data, player)          
        local card_ids = sgs.IntList()
        for _, card in sgs.qlist(skill_target:getCards("he")) do
            card_ids:append(card:getId())
        end
        skill_target:addToPile("jingnan", card_ids, false)
    end
}
...
sgs.LoadTranslationTable{
    ["zhudi"] = "朱棣",
    ["jingnan"] = "靖难",
    [":jingnan"] = "你使用杀指定目标后，你可以令其将所有牌置于武将上，此回合结束时，其获得武将上所有牌"
}
...
zouji = sgs.General(extension, "zouji", "qun", 3)  

fengjianCard = sgs.CreateSkillCard{
    name = "fengjianCard",
    target_fixed = false,--是否需要指定目标，默认false，即需要
    will_throw = true,
    filter = function(self, targets, to_select)  
        if #targets == 0 then return true end
        if #targets == 1 and targets[1] ~= to_select then return true end
        return false
        --return #targets < 2
    end,  
    ...
    on_use = function(self, room, source, targets)  
        for _, target in ipairs(targets) do
            ...
            local suits = {}  
            local all_cards = target:getHandcards()  
            ...
            local suit_count = 0  --没有的花色数
            for suit = sgs.Card_Spade, sgs.Card_Diamond do  
                if not suits[suit] then  --没有这种花色
                    --从牌堆随机获得这种花色的牌
                    local cards = sgs.IntList()  
                    for _, id in sgs.qlist(room:getDrawPile()) do  
                        local card = sgs.Sanguosha:getCard(id)  
...
fengjian = sgs.CreateZeroCardViewAsSkill{  
    name = "fengjian", 
    view_as = function(self)  
        local card = fengjianCard:clone() -- 创建虚拟牌  
        card:setSkillName(self:objectName())  
        card:setShowSkill(self:objectName())
        return card  
    end,  
    enabled_at_play = function(self, player)  
        return not player:hasUsed("#fengjianCard")
    end  
}

zouji:addSkill(fengjian)
sgs.LoadTranslationTable{  
    ["zouji"] = "邹忌",
    ["fengjian"] = "讽谏",
    [":fengjian"] = "出牌阶段限一次。你可以选择至多2名角色，令他们展示所有手牌，然后从牌堆获得手牌中缺少花色的牌各1张，因此摸牌数大于1的角色失去1点体力"
}
...
zudi = sgs.General(extension, "zudi", "wei", 3) --击揖，和沮授接近；置酒，吕雉需要酒
...
lichunfeng:addCompanion("yuantiangang")
