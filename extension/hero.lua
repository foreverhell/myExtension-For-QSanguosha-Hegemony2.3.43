...
lijianSlashCard = sgs.CreateSkillCard{  
    ...
    on_use = function(self, room, source, targets)  
        ...
        local pindian_target = room:askForPlayerChosen(source, others, "lijianSlash", "@lijianSlash-pindian", true)  
...
jujianSnatchCard = sgs.CreateSkillCard{
    ...
    on_use = function(self, room, source, targets)
        ...
        local give_target = room:askForPlayerChosen(source, room:getAlivePlayers(), self:objectName(), "@jujianSnatch")
...
yishui_vs = sgs.CreateViewAsSkill{  
    name = "yishui",  
    n = 999, -- 可以选择所有手牌  
    view_filter = function(self, selected, to_select)  
    ...
    view_as = function(self, cards)  
        ...
        slash:setSkillName("yishui")  
        slash:setShowSkill("yishui")  
        for _, card in ipairs(cards) do  
...
-- 易水触发技（处理造成伤害后的效果）  
yishui = sgs.CreateTriggerSkill{  
    name = "yishui",  
    events = {sgs.Damage},  
    view_as_skill = yishui_vs,  
    can_trigger = function(self, event, room, player, data)  
        --if not (player and player:isAlive() and player:hasSkill(self:objectName())) then return "" end  
        local damage = data:toDamage()  
        if damage.card and damage.card:getSkillName() == "yishui" and damage.to and damage.to:isAlive() and not damage.to:isKongcheng() then  
            return self:objectName()  
        end  
        return ""  
    end,  
...
gaojianli:addSkill(yishui)  
gaojianli:addSkill(lige)

sgs.LoadTranslationTable{        
    ["gaojianli"] = "高渐离",  
    ["yishui"] = "易水",
    [":yishui"] = "你可以将所有手牌当作杀使用或打出。若为使用，此杀造成伤害后，你可以弃置目标所有手牌。",
    ["lige"] = "离歌",
    [":lige"] = "任意角色回合结束时，若你没有手牌，你可以从弃牌堆随机3张牌选择1张交给任意角色"
}  
...
yinren_maxcards = sgs.CreateMaxCardsSkill{  
    ...
    extra_func = function(self, player)  
        if player:hasSkill("yinren") and not player:hasFlag("YinrenSlashUsed") then
            return 4
        end
        return 0
    end  
...
tuoying = sgs.CreateTriggerSkill{  
    ...
    on_cost = function(self, event, room, player, data, ask_who) 
        ...
        return ask_who:hasShowSkill(self:objectName()) or ask_who:askForSkillInvoke(self:objectName(), data)
    end,  
    on_effect = function(self, event, room, player, data, ask_who)  
...
xiezhan = sgs.CreateTriggerSkill{
	name = "xiezhan",
	events = {sgs.CardUsed},
	can_trigger = function(self, event, room, player, data)
		if skillTriggerable(player, self:objectName()) and event == sgs.CardUsed then
...
sgs.LoadTranslationTable{  
    ...
    ["zhengzhao_card"] = "征召",  
    ["@xiezhan-choose"] = "你可以选择一名角色，令其也对该目标出杀",  
      
    ["xiezhan"] = "协战",  
    [":xiezhan"] = "你使用杀指定目标后，你可以选择一名攻击范围内有该目标的其他角色，让其也对该目标出杀，若其出杀，其摸一张牌。",  
    ["@xiezhan-slash"] = "你可以对 %src 使用一张杀，若如此做，你摸一张牌",  
}  
...
menshen = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        if event == sgs.EventPhaseStart then  
            -- 出牌阶段开始时可以发动
            if not (player and player:isAlive() and player:getPhase() == sgs.Player_Play) then
                return ""
            end
            if player:hasSkill(self:objectName()) then
...
qingmin = sgs.CreateProhibitSkill{  --不能指定为目标，不是取消目标
    name = "qingmin",  
    is_prohibited = function(self, from, to, card)  
        if to:hasSkill(self:objectName()) and card:isKindOf("Snatch") and card:isBlack() then  
            return true  
        end  
        return false  
    end  
...
beixi = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        ...
        if use.from and use.from:isAlive() and use.from:getHandcardNum() <= 2 then  
            ...
            if beixi_player and beixi_player:isAlive() and beixi_player:hasSkill(self:objectName()) then
...
sgs.LoadTranslationTable{  
    ...
    ["junhengCard"] = "均衡",  
    ["beixi"] = "背袭",  
    [":beixi"] = "当任意其他角色使用【杀】时，若其手牌数小于等于2，你可以对其使用一张【杀】。",  
...
sgs.LoadTranslationTable{  
    ...
    ["yaoyi"] = "徭役",  
    [":yaoyi"] = "当你受到伤害后，你摸X张牌，X为场上已受伤角色的确定势力数+未确定势力数。",  
}
...
juebie = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        if all_cards:length() > 0 then  
            -- 选择一名其他角色  
            local target = room:askForPlayerChosen(dead_player, room:getOtherPlayers(dead_player), self:objectName(), "@juebie-give")  
            if target then  
                -- 将所有牌交给目标角色  
                local move = sgs.CardsMoveStruct()  
...
sgs.LoadTranslationTable{        
    ["yuji1"] = "虞姬",  
    ["juebie"] = "诀别",
    [":juebie"] = "你死亡时，你可以选择一名角色，获得你所有区域的牌"
}  
  
-- 创建武将：唐伯虎  
yuwenhuaji = sgs.General(extension, "yuwenhuaji", "qun", 4)  -- 吴国，4血，男性 
yuwenhuaji_duan = sgs.General(extension, "yuwenhuaji_duan", "qun", 4)  -- 吴国，4血，男性 
...
yuwenhuaji_duan:addSkill(cuanquan)
  
-- 翻译表  
sgs.LoadTranslationTable{  
    ["hero"] = "英雄",  
    ["yuwenhuaji"] = "宇文化及",  
    ["yuwenhuaji_duan"] = "宇文化及",  
    ["jiandi"] = "僭帝",  
    [":jiandi"] = "准备阶段，你可以失去一点体力，弃置一张牌，获得所有角色各一张牌，然后跳过摸牌阶段。",  
    ["cuanni"] = "篡逆",  
    [":cuanni"] = "你造成的伤害可以视为体力流失。",
    ["cuanquan"] = "篡权",
    [":cuanquan"] = "准备阶段，你可以弃置一张手牌，你可以选择并发起判定，直到满足以下条件：（1）判定牌和弃置的该牌颜色相同，你获得所有判定牌。（2）判定牌和弃置的该牌花色相同，你获得所有判定牌，然后跳过摸牌阶段",
...
zhuangzhou = sgs.General(extension, "zhuangzhou", "qun", 3) -- 蜀势力，4血，男性（默认）  
...
xiaoyaoMaxCards = sgs.CreateMaxCardsSkill{  
    ...
    extra_func = function(self, player)
        if not player:hasSkill("xiaoyao_maxCards") then
            return 0
        end
        local handcards = player:getHandcards()  
        local count = 0  
...
Jingsuan = sgs.CreateTriggerSkill{  
    ...
    on_cost = function(self, event, room, player, data, ask_who)  
        return player:hasShowSkill(self:objectName()) or player:askForSkillInvoke(self:objectName(),data) -- 锁定技，无需询问  
    end,  
