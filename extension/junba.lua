...
caimaozhangyun = sgs.General(extension, "caimaozhangyun", "wei", 4)
lianzhou = sgs.CreateTriggerSkill{  
    name = "lianzhou",  
    ...
    on_effect = function(self, event, room, player, data)
        ...
        local chosen_players = room:askForPlayersChosen(player, targets, self:objectName(), 0, targets:length(), "请选择任意名玩家横置", true)
        for _,p in sgs.qlist(chosen_players) do
            if p:isAlive() and p:canBeChainedBy() and not p:isChained() then
                --横置,要serverplayer类型
                room:setPlayerProperty(getServerPlayer(room, p:objectName()), "chained", sgs.QVariant(true))
            end
        end
    end
}
nulan = sgs.CreateTriggerSkill{  
    name = "nulan",  
    events = {sgs.Damage},  
    frequency = sgs.Skill_Compulsory,
    can_trigger = function(self, event, room, player, data)
    ...
    on_effect = function(self, event, room, player, data)
        if player:getHandcardNum() > player:getHp() then
            room:askForDiscard(player, self:objectName(), 4, 4, false, false)
...
caimaozhangyun:addSkill(nulan)
sgs.LoadTranslationTable{
    ["caimaozhangyun"] = "蔡瑁张允",
    ["lianzhou"] = "连舟",
    [":lianzhou"] = "锁定技。准备阶段，你横置，然后你横置任意名体力值等于你的角色",
    ["nulan"] = "驽澜",
    [":nulan"] = "锁定技。你造成伤害后，若你的手牌数：大于体力值，你弃置4张手牌；等于体力值，你弃置1张牌并恢复1点体力；小于体力值，你受到1点火焰伤害并摸5张牌"
}
-- 创建武将  
...
caoying_junba = sgs.General(extension, "caoying_junba", "wei", 4, false)
lingrenGuess = sgs.CreateTriggerSkill{  
    name = "lingrenGuess",  
    events = {sgs.CardUsed},  
    frequency = sgs.Skill_Frequent,  
    ...
    can_trigger = function(self, event, room, player, data)  
        ...
        if player:hasFlag("lingren_used") then return false end
    ...
    on_cost = function(self, event, room, player, data)
        if player:askForSkillInvoke(self:objectName(), data) then
            room:setPlayerFlag(player,"lingren_used")
            room:broadcastSkillInvoke(self:objectName())
            return true
        end
        return false  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        -- 选择一名角色  
        local target = room:askForPlayerChosen(player, targets, self:objectName(), "@lingrenGuess-select")  
        if not target then return false end
        local has_basic, has_trick, has_equip = "basic_no", "trick_no", "equip_no"
        for _, card in sgs.qlist(target:getHandcards()) do
            if card:isKindOf("BasicCard") then has_basic = "basic_yes" end
            if card:isKindOf("TrickCard") then has_trick = "trick_yes" end
            if card:isKindOf("EquipCard") then has_equip = "equip_yes" end
        end
        local guess_basic = room:askForChoice(player, self:objectName(), "basic_yes+basic_no")
        local guess_trick = room:askForChoice(player, self:objectName(), "trick_yes+trick_no")
        local guess_equip = room:askForChoice(player, self:objectName(), "equip_yes+equip_no")

        local right = 0
        if has_basic == guess_basic then right = right + 1 end
        if has_trick == guess_trick then right = right + 1 end
        if has_equip == guess_equip then right = right + 1 end

        if right >= 1 then
            local tag = sgs.QVariant()  
            tag:setValue(use.card)  
            room:setTag("lingrenGuess_card", tag)  
            
            -- 设置伤害+1的标记  
            room:addPlayerMark(player, "lingrenGuess_damage")  
            --增加伤害的目标
            room:setTag("lingrenGuess_target", sgs.QVariant(target:objectName()))
        end
        if right >= 2 then
            player:drawCards(2,self:objectName())
        end
        if right >= 3 and (not player:hasSkill("jianxiong") or not player:hasSkill("xingshang")) then
            room:loseMaxHp(player,1)
            if not player:hasSkill("jianxiong") then room:handleAcquireDetachSkills(player, "jianxiong") end
            if not player:hasSkill("xingshang") then room:handleAcquireDetachSkills(player, "xingshang") end
        end
        return false  
    end  
}  
...
-- 增加伤害的触发技  
lingrenGuessDamage = sgs.CreateTriggerSkill{  
    name = "#lingrenGuess-damage",  --附加效果用#号，将技能解耦
    events = {sgs.DamageCaused},  
    global = true,  
    can_trigger = function(self, event, room, player, data)  
        if not player or player:isDead() or player:getMark("lingrenGuess_damage") <= 0 then  
            return false  
        end  
        local damage = data:toDamage()  
        local card = room:getTag("lingrenGuess_card"):toCard()  
        local target_name = room:getTag("lingrenGuess_target"):toString()  
    ...
    on_effect = function(self, event, room, player, data)  
        room:removePlayerMark(player, "lingrenGuess_damage")  
        room:removeTag("lingrenGuess_card")  
        room:removeTag("lingrenGuess_target")
...
caoying_junba:addSkill(lingrenGuess)
caoying_junba:addSkill(lingrenGuessDamage)
extension:insertRelatedSkills("lingrenGuess", "#lingrenGuess-damage")
sgs.LoadTranslationTable{
    ["caoying_junba"] = "曹婴",
    ["lingrenGuess"] = "凌人",
	[":lingrenGuess"] = "每回合限一次。当你使用伤害牌指定目标后，你可以猜测其中一个目标手牌中是否有基本牌、锦囊牌、装备牌，至少猜对：1项，此牌对其伤害+1；2项，你摸2张牌；3项，你失去一点体力上限，获得【奸雄】【行殇】",
    ["basic_yes"] = "有基本牌",
    ["basic_no"] = "没有基本牌",
    ["trick_yes"] = "有锦囊牌",
    ["trick_no"] = "没有锦囊牌",
...
hudingjin = sgs.General(extension, "hudingjin", "shu", 4)
deshi = sgs.CreateTriggerSkill{
    name = "deshi",
    events = {sgs.DamageInflicted},
    can_trigger = function(self, event, room, player, data)
        ...
        local damage = data:toDamage()
        if damage.card and damage.card:isKindOf("Slash") and player:isWounded() then
            return self:objectName()
        end
        return false
    end,  
...
wuyuan = sgs.CreateViewAsSkill{  
    ...
    enabled_at_play = function(self, player)  
        return not player:hasUsed("#wuyuanCard")  
    end  
}
hudingjin:addSkill(deshi)
hudingjin:addSkill(wuyuan)
sgs.LoadTranslationTable{
	["hudingjin"] = "胡定金",
	["deshi"] = "德释",
	[":deshi"] = "当你受到杀的伤害时，若你已受伤，你可以防止此伤害，并获得此杀，然后减少1点体力上限",
	["wuyuan"] = "武缘",
	[":wuyuan"] = "出牌阶段限一次。你可以将1张杀交给一名其他角色，然后你加1点体力上限，并恢复1点体力",
}
...
jikang1 = sgs.General(extension, "jikang1", "wei", 3)  -- 吴国，4血，男性  
qingxianHP = sgs.CreateTriggerSkill{  
    name = "qingxianHP",  
    ...
    on_effect = function(self, event, room, player, data)            
        ...
        local choices = {"losehp_equip"}    
        if target:isWounded() or not target:getEquips():isEmpty() then  
            table.insert(choices, "recover_discard")  
        end  
        local choice = room:askForChoice(player, self:objectName(), table.concat(choices, "+"))
        if choice == "recover_discard" then
            local recover = sgs.RecoverStruct()  
            recover.who = player  
            -- 执行回复  
            room:recover(target, recover)
        ...
        elseif choice == "losehp_equip" then
            room:loseHp(target, 1)
...
jikang1:addSkill(qingxianHP)
sgs.LoadTranslationTable{
    ["jikang1"] = "嵇康",
    ["qingxianHP"] = "清弦",
    [":qingxianHP"] = "你受到伤害/恢复体力后，你可以选择对伤害来源/一名其他角色（1）令其恢复1点体力，并弃置其装备区1张装备牌（2）失去一点体力，并使用摸牌堆一张装备牌。若使用或弃置的牌是梅花，你摸一张牌",
    ["recover_discard"] = "回复1点体力并弃置1张装备区的装备",
    ["losehp_equip"] = "失去1点体力并使用1张摸牌堆的装备"
}
...
lisu = sgs.General(extension, "lisu", "qun", 3)  -- 吴国，4血，男性  

qiaoyan = sgs.CreateTriggerSkill{
    name = "qiaoyan",
    events = {sgs.DamageInflicted},
    frequency = sgs.Skill_Compulosry,
    can_trigger = function(self, event, room, player, data)
        if not player or not player:isAlive() or not player:hasSkill(self:objectName())  then
            return ""
        end
        local zhu_pile = player:getPile("zhu")
        local damage = data:toDamage()
        if zhu_pile:length() == 0 or damage.from then --没有珠；或者有珠，并且有伤害来源
            return self:objectName()
        end
        return ""
    end,
    ...
    on_effect = function(self, event, room, player, data)
        ...
        if zhu_pile:length() == 0 then --没有珠
            ...
            if damage.from then
                local first_card_id = zhu_pile:first()  
...
qiaoying = sgs.CreateTriggerSkill{  
    name = "qiaoying",  
    events = {sgs.EventPhaseStart},  
    frequency = sgs.Skill_Compulosry,
    can_trigger = function(self, event, room, player, data)  
        if not (player and player:isAlive() and player:hasSkill(self:objectName()) and player:getPhase() == sgs.Player_Play) then
            return ""
        end
        local zhu_pile = player:getPile("zhu")  
        if zhu_pile:length() > 0 then --有珠
            return self:objectName()
        end
        return ""  
    end,  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        if target ~= player then
            local victim = room:askForPlayerChosen(player, room:getOtherPlayers(target), self:objectName())
            local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)  
            slash:setSkillName(self:objectName())  
            local use = sgs.CardUseStruct()  
            use.card = slash  
...
lisu:addSkill(qiaoying)
sgs.LoadTranslationTable{
    ["lisu"] = "李肃",
    ["qiaoyan"] = "巧言",
    [":qiaoyan"] = "锁定技。你受到伤害时，若你没有“珠”，你摸一张牌并将一张牌放在武将牌上，称为“珠”，然后防止此伤害；若你有“珠”，伤害源获得“珠”",
    ["qiaoying"] = "瞧影",
    [":qiaoying"] = "锁定技。你的出牌阶段开始时，若你有“珠”，你令一名角色获得“珠”，若该角色不为你，其视为对你指定的另一名角色使用一张杀"
}

liujinliupei = sgs.General(extension, "liujinliupei", "wei", 3, false)  -- 吴国，4血，男性  
...
quyi_junba = sgs.General(extension, "quyi_junba", "qun", 4)
fuqi = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)
        ...
        if event == sgs.CardUsed then
            local use = data:toCardUse()
            if use.card:getTypeId() == sgs.Card_TypeSkill then return "" end
            if use.from == player and not use.card:isKindOf("Jink") then
                return self:objectName()
            end
        end
        return ""
    ...
    on_effect = function(self, event, room, player, data)
        ...
        for _,p in sgs.qlist(room:getAlivePlayers()) do
            if player:distanceTo(p) <= 1 or p:distanceTo(player) <= 1 then
...
quyi_junba:addSkill(fuqi)
quyi_junba:addSkill(jiaozi)
sgs.LoadTranslationTable{
    ["quyi_junba"] = "麹义",
    ["fuqi"] = "伏骑",
    [":fuqi"] = "锁定技。你使用牌时，你到其距离为1或到你距离为1的角色不能响应此牌",
    ["jiaozi"] = "骄恣",
    [":jiaozi"] = "锁定技。你造成或受到伤害时，若你的手牌数全场唯一最多，此伤害+1",
}
-- 创建武将蒙恬  
...
simahui = sgs.General(extension, "simahui", "qun", 3)  
...
longfeng = sgs.CreateTriggerSkill{  
    ...
    --limit_mark = "@longfeng",
    can_trigger = function(self, event, room, player, data)  
        local simahui = room:findPlayerBySkillName(self:objectName())  
        if not simahui or not simahui:isAlive() or not simahui:hasSkill(self:objectName()) then return "" end  

        if event == sgs.EventPhaseStart then  
            if player == simahui and player:getPhase() == sgs.Player_Start and player:getMark("@longfeng") == 0 then  
                return self:objectName(),simahui:objectName()
            end 
...
simahui:addSkill(longfeng)  
simahui:addSkill(longfengYehuo)  
...
sunhong = sgs.General(extension, "sunhong", "wu", 4)
jianrun = sgs.CreateTriggerSkill{  
    name = "jianrun",  
    ...
    on_effect = function(self, event, room, player, data)
        ...
        if targets:isEmpty() then return false end
        local target = room:askForPlayerChosen(player,targets,self:objectName())
        if target and not target:getEquips():isEmpty() then
            local card_id = room:askForCardChosen(player,target,"e",self:objectName())
            if card_id then
...
xianboCard = sgs.CreateSkillCard{  
    name = "xianboCard",  
    skill_name = "xianbo",  
    target_fixed = false,  
    will_throw = true,
    ...
    on_use = function(self, room, source, targets)  
        local target = targets[1]
        local num = target:getEquips():length()
        if source:getHandcardNum() > num then
            local n = source:getHandcardNum()-num
            room:askForDiscard(source,self:objectName(),n,n,false,false)
        elseif source:getHandcardNum() < num then
            source:drawCards(num-source:getHandcardNum(), self:objectName())
        end
    end  
}  
  
xianbo = sgs.CreateZeroCardViewAsSkill{  
    name = "xianbo",  
    view_as = function(self)  
        local card = xianboCard:clone()  
        card:setShowSkill(self:objectName())  
        return card  
    end,  
    enabled_at_play = function(self, player)  
        return not player:hasUsed("#xianboCard")  
    end  
}
sunhong:addSkill(jianrun)
sunhong:addSkill(xianbo)
sgs.LoadTranslationTable{
    ["sunhong"] = "孙弘",
    ["jianrun"] = "僭润",
    [":jianrun"] = "摸牌阶段，你可以改为获得一名其他角色装备区的一张装备",
    ["xianbo"] = "险波",
    [":xianbo"] = "出牌阶段限一次。你可以将手牌调整至与一名角色装备区数量相等",
}
...
xing_taishici = sgs.General(extension, "xing_taishici", "wu", 4)  

chongweiDistance = sgs.CreateDistanceSkill{  
    name = "#chongwei-distance",  
    correct_func = function(self, from, to)  
        if to:hasShownSkill("chongwei") then  
            local mark = to:getMark("@wei_mark")  
            if mark > 0 then  
                return mark  
            end  
        end  
...
-- 重围触发技能部分  
chongwei = sgs.CreateTriggerSkill{  
    name = "chongwei",  
    frequency = sgs.Skill_Compulsory,  
    events = {sgs.GeneralShown, sgs.Damage},  
      
    can_trigger = function(self, event, room, player, data)  
        if event == sgs.GeneralShown then  
            if player:hasShownSkill(self:objectName()) then  
                --local is_head = data:toBool()  
                -- is_head 为 true 表示主将被明置,false 表示副将被明置  
                return self:objectName()  
            end  
        elseif event == sgs.Damage then  
            if player:hasShownSkill(self:objectName()) and player:getMark("@wei_mark") > 0 then  
                return self:objectName()  
            end  
        end  
        return ""  
    ...
    on_effect = function(self, event, room, player, data)  
        if event == sgs.GeneralShown then  
            -- 明置时获得3个"围"标记  
            room:setPlayerMark(player, "@wei_mark", 3)  
        elseif event == sgs.Damage then  
            -- 造成伤害后减少1个"围"标记  
            local mark = player:getMark("@wei_mark") - 1
            room:setPlayerMark(player, "@wei_mark", mark)  
...
chongzu = sgs.CreateTriggerSkill{  
    name = "chongzu",  
    events = {sgs.TargetConfirming},  
    frequency = sgs.Skill_Frequent,      
    can_trigger = function(self, event, room, player, data)  
        ...
        if player:hasFlag("chongzu_used") then return "" end
        local use = data:toCardUse()  
    ...
    on_effect = function(self, event, room, player, data)  
        room:setPlayerFlag(player, "chongzu_used")  
        player:drawCards(2)  
        return false  
    end  
}  
...
-- 插入技能关联  
sgs.insertRelatedSkills(extension, "chongwei", "#chongwei-distance")

xing_taishici:addSkill(chongwei)
xing_taishici:addSkill(chongweiDistance)
xing_taishici:addSkill(chongzu)
sgs.LoadTranslationTable{
["#xing_taishici"] = "星之围困",  
["xing_taishici"] = "星太史慈",  
["illustrator:xing_taishici"] = "Your Artist Name",  
["chongwei"] = "重围",  
[":chongwei"] = "锁定技。当你明置该武将时，你获得3个'围'标记。其他角色到你的距离+X，X为'围'标记的数量。当你造成伤害后，'围'标记-1，当'围'标记等于0时，你恢复1点体力并摸当前体力值张牌，然后失去此技能。",  
["wei_mark"] = "围",  
["chongzu"] = "冲阻",  
[":chongzu"] = "每回合限一次。你使用牌指定自己为目标时，你可以摸2张牌。",  
["@chongzu-draw"] = "是否发动'冲阻'，摸2张牌",
}

xing_yuanshao = sgs.General(extension, "xing_yuanshao", "qun", 4)  
xiaoyan = sgs.CreateTriggerSkill{
    name = "xiaoyan",
    ...
    can_trigger = function(self, event, room, player, data)
        ...
        if event == sgs.Death then
            room:setPlayerMark(player, "@xiaoyan_death", 1) --有人死亡，加标记
        elseif event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start then
            if player:getMark("@xiaoyan_death") == 0 then --没有人死亡，触发技能
                return self:objectName()
            else
                room:setPlayerMark(player, "@xiaoyan_death", 0) --有人死亡，不触发技能，清除标记
            end
        end
        return false
    end,  
    ...
    on_effect = function(self, event, room, player, data)
        for _, p in sgs.qlist(room:getOtherPlayers(player)) do
            ...
            local card = room:askForCard(p, ".", "@xiaoyan-give:" .. p:objectName(), sgs.QVariant(), sgs.Card_MethodNone)  
            if card then  
                room:obtainCard(player, card, false)
                local recover = sgs.RecoverStruct()
                recover.who = p
        ...
        if player:getMark("xiaoyan_used") > 0 then
            room:loseHp(player,1)
        else
            room:setPlayerMark(player,"xiaoyan_used",1)
        end
        return false
    end  
}  
xing_yuanshao:addSkill(xiaoyan)
sgs.LoadTranslationTable{
	["xing_yuanshao"] = "星袁绍",
	["xiaoyan"] = "硝烟",
	[":xiaoyan"] = "锁定技。你的准备阶段，若上一轮没有角色死亡，所有其他角色受到你造成的1点火焰伤害，然后这些角色可以交给你一张牌并恢复1点体力。若本局游戏你不是第一次使用此技能，你失去1点体力",
}
...
xuncai = sgs.General(extension, "xuncai", "qun", 3, false)  -- 吴国，4血，男性  
lieshiCard = sgs.CreateSkillCard{  
    name = "lieshiCard",  
    will_throw = true,
    filter = function(self, targets, to_select)  
        return #targets == 0 and to_select:objectName() ~= sgs.Self:objectName()
    ...
    on_use = function(self, room, source, targets)  
        local target = targets[1]  
        local my_card = sgs.Sanguosha:getCard(self:getSubcards():first())
        local to_discard_type = ""
        if my_card:isKindOf("Slash") then
            to_discard_type = "Jink"
        elseif my_card:isKindOf("Jink") then
...
lieshi = sgs.CreateViewAsSkill{  
    name = "lieshi",  
    n = 999,  
    view_filter = function(self, selected, to_select)  
        if #selected == 0 then  
            return not to_select:hasFlag("using") and (to_select:isKindOf("Slash") or to_select:isKindOf("Jink"))
        else  
            return not to_select:hasFlag("using") and (to_select:isKindOf("Slash") or to_select:isKindOf("Jink")) 
            and to_select:isKindOf("Slash") == selected[1]:isKindOf("Slash")
        end  
    end,  
    view_as = function(self, cards)  
        ...
        local slash_count = 0  
        local jink_count = 0
        for _, hand_card in sgs.qlist(hand_cards) do  --cards只可能是杀或闪，手牌不一定是杀或闪
            if hand_card:isKindOf("Slash") then  
                slash_count = slash_count + 1  
            elseif hand_card:isKindOf("Jink") then
                jink_count = jink_count + 1
            end  
        end  
        if cards[1]:isKindOf("Slash") and #cards ~= slash_count then return nil end
        if cards[1]:isKindOf("Jink") and #cards ~= jink_count then return nil end
          
        local new_card = lieshiCard:clone()          
        for _, card in ipairs(cards) do  
    ...
    enabled_at_play = function(self, player)
        return not player:isKongcheng() and not player:hasUsed("#lieshiCard")
    end  
}  
yuanxian = sgs.CreateTriggerSkill{  
    name = "yuanxian",  
    events = {sgs.Dying},  
    frequency = sgs.Skill_Compulosry,
    can_trigger = function(self, event, room, player, data)  
...
zhangxuan = sgs.General(extension, "zhangxuan", "wu", 4)  
...
sgs.LoadTranslationTable{
	["zhangxuan"] = "张璇",
	["tongli"] = "同礼",
	[":tongli"] = "出牌阶段，当你使用牌指定目标后，若你手牌中的花色数等于你此阶段已使用的牌数，你可以令此牌额外执行1次。",
}

zhaoxiang = sgs.General(extension, "zhaoxiang", "shu", 4)
fanghunVS = sgs.CreateZeroCardViewAsSkill{  
    name = "fanghun",  
    view_as = function(self)
        local card_name = sgs.Self:getTag(self:objectName()):toString()
		if card_name ~= "" then
    ...
    enabled_at_play = function(self, player)   
        return player:getMark("@mei") > 0
    end,

    enabled_at_response = function(self, player, pattern)
        return player:getMark("@mei") > 0 and (pattern == "slash" or pattern == "jink" or string.find(pattern, "peach") or string.find(pattern, "analeptic"))
    end,
...
fanghun = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)
        if event == sgs.TargetConfirming then
            local fanghun_player = room:findPlayerBySkillName(self:objectName())  
            if not (fanghun_player and fanghun_player:isAlive() and fanghun_player:hasSkill(self:objectName())) then return "" end
            ...
            -- 检查是否为使用者或目标  
            if source and source:objectName() == fanghun_player:objectName() then  
                -- 技能拥有者使用牌指定其他角色  
                for _, target in sgs.qlist(use.to) do  
                    if target:objectName() ~= fanghun_player:objectName() then  
                        is_involved = true  
                        other_player = target  
                        break  
                    end  
                end  
            elseif source and source:objectName() ~= fanghun_player:objectName() then  
                -- 其他角色使用牌指定技能拥有者  
                for _, target in sgs.qlist(use.to) do  
                    if target:objectName() == fanghun_player:objectName() then  
                        is_involved = true  
            ...
            if is_involved then
                return self:objectName(), fanghun_player:objectName()
            end
        elseif event == sgs.CardUsed then
            local use = data:toCardUse()
            if use.card:getSkillName() == self:objectName() then
                room:setPlayerMark(use.from, "@mei", use.from:getMark("@mei")-1)
    ...
    on_effect = function(self, event, room, player, data, ask_who)  
        room:addPlayerMark(ask_who, "@mei")        
        return false  
    end  
}  
...
sgs.LoadTranslationTable{
	["zhaoxiang"] = "赵襄",
	["fanghun"] = "芳魂",
	[":fanghun"] = "你使用杀指定目标或成为杀的目标后，你获得1个“梅影”标记；你可以将一个“梅影”标记当作一张基本牌使用，然后摸1张牌",
}
...
zhujun = sgs.General(extension, "zhujun", "wu", 4)  -- 吴国，4血，男性  
