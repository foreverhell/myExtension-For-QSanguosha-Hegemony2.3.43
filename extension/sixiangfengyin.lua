...
yuqi = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)
        ...
        if owner:distanceTo(damage.to) <= owner:getHp() then
...
xingfa = sgs.CreateTriggerSkill{  
    ...
    frequency = sgs.Skill_Frequent,
    can_trigger = function(self, event, room, player, data)
        if not (player and player:isAlive() and player:hasSkill(self:objectName())) return "" end
...
yunji = sgs.CreateOneCardViewAsSkill{  
    name = "yunji",  
    filter_pattern = "EquipCard",  
    view_as = function(self, card)  
        local Collateral = sgs.Sanguosha:cloneCard("Collateral", card:getSuit(), card:getNumber())  
        Collateral:addSubcard(card:getId())  
        Collateral:setSkillName(self:objectName())  --设置转化牌的技能名
        Collateral:setShowSkill(self:objectName())  --使用时亮将
        return Collateral  
    end,
    enabled_at_play = function(self, player)  
        return true
...
wangyun_feng:addSkill(yunji)
sgs.LoadTranslationTable{
    ["wangyun_feng"] = "王允",
    ["zongji"] = "纵计",
    [":zongji"] = "一名角色受到杀或决斗的伤害后，你可以弃置其与伤害来源各1张牌",
    ["yunji"] = "运机",
    [":yunji"] = "你可以将一张装备牌当借刀杀人使用",
}
...
tainfeng_feng:addSkill(gangjian)
tainfeng_feng:addSkill(guijie)
sgs.LoadTranslationTable{
    ["tainfeng_feng"] = "田丰",
    ["gangjian"] = "刚谏",
