...
gue = sgs.CreateOneCardViewAsSkill{  
    ...
    view_as = function(self, card)  
        -- 检查是否为最后一张手牌  
        if sgs.Self:getHandcardNum() == 1 then  
            -- 可以选择视为杀或闪  
...
-- 创建武将：
liru = sgs.General(extension, "liru", "qun", 3)  -- 吴国，4血，男性  
...
Tianming = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        if to_discard > 0 then  
            room:askForDiscard(player, self:objectName(), to_discard, to_discard, false, true)  
        end  
...
luyusheng_canghai = sgs.General(extension, "luyusheng", "wu", 3, false)  
...
xianjing = sgs.CreateZeroCardViewAsSkill{  
    name = "xianjing",  
    view_as = function(self)  
        local slash = sgs.Sanguosha:cloneCard("slash", sgs.Card_NoSuit, 0)  
        slash:setSkillName(self:objectName())  
    ...
    enabled_at_play = function(self, player)  
        return not player:hasUsed("#xianjing") and sgs.Slash_IsAvailable(player) and not player:isKongcheng()
    end,  
      
    enabled_at_response = function(self, player, pattern)  
        return not player:hasUsed("#xianjing") and (pattern == "slash" or pattern == "jink") and not player:isKongcheng()
    end  
}  
...
sgs.LoadTranslationTable{  
    ...
    ["luzhi"] = "鲁芝",  
    ["xianjing"] = "衔镜",  
    [":xianjing"] = "每轮限一次。当你需要使用杀或闪时，若你有手牌，你可以视为使用之。",  
    ["qingzhong"] = "清忠",  
    ...
    on_use = function(self, room, source, targets)  
        ...
        if choice == "junxing_discard" then
            ...
            if #valid_cards > 0 then  
                ...
                chosen_id = room:askForCardChosen(target, target, "h", self:objectName(), false, sgs.Card_MethodDiscard, invalid_cards)  
                if chosen_id then                
...
yuce = sgs.CreateTriggerSkill{  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        if not damage.from:isKongcheng() and #valid_cards > 0 then      
            card_id = room:askForDiscard(damage.from, self:objectName(), 1, 1, false, true, "@yuce-discard")  
            if card_id then                
...
qianya = sgs.CreateTriggerSkill{
    ...
    can_trigger = function(self, event, room, player, data)  
        ...
        if event == sgs.DamageInflicted then -- and TriggerSkill.triggerable(player, self) 
            return self:objectName()     
...
zhenlie = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)  
        ...
        if effect.to and effect.to:hasSkill(self:objectName())   
           and effect.card and (effect.card:isKindOf("Slash") or effect.card:isKindOf("TrickCard")) then  
            return self:objectName()  
        end  
    end,  
    ...
    on_effect = function(self, event, room, player, data)  
        ...
        local effect = data:toCardEffect()  
        -- 弃置来源一张牌
        if effect.from and effect.from:isAlive() and not effect.from:isAllNude() then  
            local card_id = room:askForCardChosen(player, effect.from, "hej", self:objectName())  
...
sgs.LoadTranslationTable{
    ["wangyi"] = "王异",
    ["zhenlie"] = "贞烈",
    [":zhenlie"] = "你成为杀或锦囊的目标时，你可以失去一点体力并取消之，然后摸一张牌，弃置来源一张牌",
    ["miji"] = "秘计",  
    [":miji"] = "弃牌阶段结束后，你可以摸X张牌，X为你已损失的体力值。",  
    ["#mijiDraw"] = "%from 发动了【%arg2】，摸了 %arg 张牌",  
}

wenyuan = sgs.General(extension, "wenyuan", "wei", 3) -- 吴苋，蜀势力，3血，女性
...
duoqi = sgs.CreateTriggerSkill{  
    ...
    can_trigger = function(self, event, room, player, data)   
        ...
        if player and player:getHandcardNum() == 0 then
            wenyuan = room:findPlayerBySkillName(self:objectName())
            if wenyuan and wenyuan:isAlive() and wenyuan:getMark("@duoqi") > 0 then
                return self:objectName(), wenyuan:objectName()
            end
        end  
        return ""  
    end,  
    on_cost = function(self, event, room, player, data, ask_who)  
        if ask_who:askForSkillInvoke(self:objectName(), data) then  
            room:setPlayerMark(player, "@duoqi", 0)  
            room:broadcastSkillInvoke(self:objectName(), player)  
            return true  
        end  
        return false  
...
wenyuan:addSkill(qili)  
wenyuan:addSkill(duoqi)  
...
-- 创建武将：
yufan = sgs.General(extension, "yufan", "wu", 3)  -- 吴国，4血，男性  
...
sgs.LoadTranslationTable{
    ...
    ["zhiyan"] = "直言",  
    [":zhiyan"] = "结束阶段，你可以令一名角色摸一张牌并展示之，若为装备牌，其使用之并恢复一点体力。",  
    ["@zhiyan-invoke"] = "你可以发动直言",  
    ["~zhiyan"] = "选择一名角色",
}
...
fushuCard = sgs.CreateSkillCard{  
    ...
    on_use = function(self, room, source, targets)  
        ...
        for _, p in sgs.qlist(room:getOtherPlayers(target)) do  
            ...
            elseif p:getKingdom() ~= target:getKingdom() then
