...
qi_hejin = sgs.General(extension, "qi_hejin", "qun", 4)  
...
qi_hejin:addSkill(zhaobing)  
qi_hejin:addSkill(zhuhuan4)  
qi_hejin:addSkill(yanhuoDeath)  
qi_hejin:addSkill(yanhuoDeathDamage)
sgs.LoadTranslationTable{
["#qi_hejin"] = "大将军",  
["qi_hejin"] = "起何进",  
["illustrator:qi_hejin"] = "待定",  
["zhaobing"] = "诏兵",  
...
qi_wangrong = sgs.General(extension, "qi_wangrong", "qun", 3, false)  
fengzi = sgs.CreateTriggerSkill{
    ...
    can_trigger = function(self, event, room, player, data)
        if player and player:isAlive() and player:hasSkill(self:objectName()) then
            if player:isKongcheng() or player:hasFlag("fengzi_used") then return "" end
            local use = data:toCardUse()
            if use.from == player then
                if use.card:isKindOf("BasciCard") or use.card:isNDTrick() then
                    return self:objectName()
                end
            end
        end
    ...
    on_cost = function(self, event, room, player, data)
        ...
        if room:askForCard(player, pattern, "@fengzi-discard", data, sgs.Card_MethodDiscard) then
            room:setPlayerFlag(player,"fengzi_used")
            return true
        end
    end,  
    on_effect = function(self, event, room, player, data)
...
jizhan = sgs.CreateTriggerSkill{
    ...
    on_effect = function(self, event, room, player, data)
        ...
        local show_card_ids = room:getNCards(1)
        all_cards:append(show_card_ids:first())
        local last_number = sgs.Sanguosha:getCard(show_card_ids:first()):getNumber()
        ...
        move.card_ids = show_card_ids  
        move.reason = sgs.CardMoveReason(sgs.CardMoveReason.S_REASON_DEMONSTRATE, player:objectName())  
        -- 执行移动并展示  
        room:moveCardsAtomic(move, true)
        repeat
            local choice = room:askForChoice(player,self:objectName(),"greater+less")

            local show_card_ids = room:getNCards(1)
            all_cards:append(show_card_ids:first())
            local now_number = sgs.Sanguosha:getCard(show_card_ids:first()):getNumber()
            local move = sgs.CardsMoveStruct()  
            move.from = nil  
            move.from_place = sgs.Player_DrawPile  
            move.to = nil  -- 移动到桌面  
            ...
            room:moveCardsAtomic(move, true)
            is_right = false
            if choice == "greater" and now_number > last_number then
                is_right = true
            elseif choice == "less" and now_number < last_number then
                is_right = true
            end
...
qi_wangrong:addSkill(fengzi)
qi_wangrong:addSkill(jizhan)
sgs.LoadTranslationTable{  
    ["qi_wangrong"] = "起王荣",  
      
    ["fengzi"] = "丰姿",  
    [":fengzi"] = "出牌阶段限一次，当你使用基本牌或普通锦囊牌时，你可以弃置一张类别相同的手牌，令此牌结算两次",      
    ["jizhan"] = "吉占",  
    [":jizhan"] = "摸牌阶段，你可以改为展示牌堆顶一张牌，然后猜测下一张牌的点数大于或小于此牌，然后展示之，若猜测正确，则重复此流程，否则你获得此次所有以此法展示的牌", 
}
...
cheng_chunyuqiong = sgs.General(extension, "cheng_chunyuqiong", "qun", 4)
cangchu = sgs.CreateTriggerSkill{
	name = "cangchu",
	events = {sgs.CardsMoveOneTime, sgs.EventPhaseStart, sgs.EventPhaseEnd},
    frequency = sgs.Skill_Frequent,  
    can_trigger = function(self, event, room, player, data)
		if skillTriggerable(player, self:objectName()) and event == sgs.CardsMoveOneTime and player:getMark("cangchu_unable")==0 then
				...
				for _, move_data in sgs.qlist(move_datas) do
					...
					if move.to_place == sgs.Player_PlaceHand then
						if move.to and move.to:isAlive() and player:objectName()==move.to:objectName() then
                            room:addPlayerMark(player,"@cangchu",move.card_ids:length())
						end
					end
                end
			end
        elseif event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Finish then
            ...
            if owner and owner:isAlive() and owner:hasSkill(self:objectName()) and owner:getMark("cangchu_unable")==0 and owner:getMark("@cangchu") > 0 then
        ...
        elseif event == sgs.EventPhaseEnd and player:getPhase() == sgs.Player_Finish and player:hasSkill(self:objectName()) then
            room:setPlayerMark(player,"cangchu_unable",0)
		end
		return ""
	end,
    on_cost = function(self, event, room, player, data, ask_who)  
        ...
        room:setPlayerMark(ask_who,"@cangchu",0)
        return false
    end,  
            
    on_effect = function(self, event, room, player, data, ask_who)
        local n = ask_who:getMark("@cangchu")
        local chosen_players = room:askForPlayersChosen(ask_who, room:getAlivePlayers(), self:objectName(), 0, n, "请选择玩家", false)
        -- 让选中的玩家各摸一张牌  
...
shishou = sgs.CreateTriggerSkill{
    name = "shishou",
    ...
    on_effect = function(self, event, room, player, data)
        ...
        elseif event == sgs.Damaged then
            room:setPlayerMark(player,"cangchu_unable",1)
        end
    end
}
cheng_chunyuqiong:addSkill(cangchu)
cheng_chunyuqiong:addSkill(shishou)
sgs.LoadTranslationTable{
    ["cheng_chunyuqiong"] = "淳于琼",
    ["cangchu"] = "仓储",
    [":cangchu"] = "每个结束阶段开始时，你可以令至多X名角色各摸一张牌（X为你此回合获得牌的数量）",--若X大于存活角色数，则改为各摸2张牌
    ["shishou"] = "失守",
    [":shishou"] = "锁定技。当你使用酒时，你摸3张牌，然后你此回合不能使用牌；当你受到火焰伤害后，”仓储“失效直到你下回合结束"
}
cheng_gaolan = sgs.General(extension, "cheng_gaolan", "qun", 4)  -- 吴国，4血，男性  
...
cheng_gaolan:addSkill(jungong)
cheng_gaolan:addSkill(dengli)
sgs.LoadTranslationTable{
    ["cheng_gaolan"] = "高览",
    ["jungong"] = "峻攻",
...
cheng_guanyu = sgs.General(extension, "cheng_guanyu", "shu", 5)

guanjue = sgs.CreateTriggerSkill{
    name = "guanjue",
...
nianenVS = sgs.CreateOneCardViewAsSkill{  
    name = "nianen",  
    guhuo_type = "b",  -- 显示基础牌选择框  
    filter_pattern = ".",  --牌名，类型|颜色，花色|点数|位置
    response_or_use = true,  
    view_as = function(self, card)  
        local pattern = sgs.Self:getTag(self:objectName()):toString()  
        local new_card = sgs.Sanguosha:cloneCard(pattern, card:getSuit(), card:getNumber())  
        if new_card then
    ...
    enabled_at_play = function(self, player)  
        return not player:hasFlag("nianen_unable") and not player:isNude() 
    end,
    
    enabled_at_response = function(self, player, pattern)  
        return not player:hasFlag("nianen_unable") and not player:isNude() and 
        (pattern == "slash" or pattern == "jink" or string.find(pattern, "peach") or string.find(pattern, "analeptic"))
    end  
}


nianen = sgs.CreateTriggerSkill{
    name = "nianen",
    events = {sgs.CardUsed, sgs.CardResponded},
    view_as_skill = nianenVS,
    ...
    can_trigger = function(self, event, room, player, data)  
        if skillTriggerable(player, self:objectName()) and (event == sgs.CardUsed or event == sgs.CardResponded) then
            ...
            if card and card:getSkillName() == self:objectName() then
                local subcard_id = card:getSubcards():first()
                local subcard = sgs.Sanguosha:getCard(subcard_id)
            
                if not card:isKindOf("Slash") or not subcard:isRed() then
                    room:setPlayerFlag(player,"nianen_unable")
                end
            end
        end
    end
}
cheng_guanyu:addSkill(guanjue)
cheng_guanyu:addSkill(nianen)
sgs.LoadTranslationTable{
    ["cheng_guanyu"] = "承关羽",
    ["guanjue"] = "冠绝",
    [":guanjue"] = "锁定技。当你使用或打出牌时，你令其他所有角色本回合不能使用或打出与之相同花色的牌",
    ["nianen"] = "念恩",
    [":nianen"] = "你可以将一张牌当任意基本牌使用或打出，若你以此法转化后的牌不为【杀】或此牌不为红色，本回合此技能失效"
}
cheng_quyi = sgs.General(extension, "cheng_quyi", "qun", 4)
...
cheng_quyi:addSkill(fuqi)
cheng_quyi:addSkill(jiaozi)
sgs.LoadTranslationTable{
    ["cheng_quyi"] = "麹义",
    ["fuqi"] = "伏骑",
...
zhuan_caimaozhangyun = sgs.General(extension, "zhuan_caimaozhangyun", "wei", 4)
...
zhuan_caimaozhangyun:addSkill(lianzhou)
zhuan_caimaozhangyun:addSkill(nulan)
sgs.LoadTranslationTable{
    ["zhuan_caimaozhangyun"] = "蔡瑁张允",
    ["lianzhou"] = "连舟",
...
chentai = sgs.General(extension, "chentai", "wei", 4, true)

jiuxian4_card = sgs.CreateSkillCard{
    name = "jiuxian4Card",
    target_fixed = false,
    will_throw = false,
    handling_method = sgs.Card_MethodNone,
    ...
    filter = function(self, targets, to_select, Self)
        if #targets > 0 or to_select:objectName() == Self:objectName() then return false end
        local duel = sgs.Sanguosha:cloneCard("duel", sgs.Card_NoSuit, 0)
        local selected = sgs.PlayerList()
        local valid = duel:targetFilter(selected, to_select, Self)
            and not Self:isProhibited(to_select, duel)
        duel:deleteLater()
        return valid
    end,
    ...
    on_use = function(self, room, source, targets)
        ...
        local reason = sgs.CardMoveReason(sgs.CardMoveReason_S_REASON_RECAST,
            source:objectName(), "", "jiuxian4", "")
        local move = sgs.CardsMoveStruct(ids, nil, sgs.Player_DiscardPile, reason)
        room:moveCardsAtomic(move, true)
        source:drawCards(recast_count, "jiuxian4")

        local target = targets[1]
        if not (source:isAlive() and target and target:isAlive()) then return end
        local duel = sgs.Sanguosha:cloneCard("duel", sgs.Card_NoSuit, 0)
        duel:setSkillName("jiuxian4")
        duel:setShowSkill("jiuxian4")
        local use = sgs.CardUseStruct()
        use.card = duel
        use.from = source
        use.to:append(target)
...
jiuxian4VS = sgs.CreateViewAsSkill{
    name = "jiuxian4",
    n = 999,

    view_filter = function(self, selected, to_select)
        local required = math.ceil(sgs.Self:getHandcardNum() / 2)
        return #selected < required and not to_select:isEquipped()
            and not sgs.Self:isJilei(to_select)
    end,

    view_as = function(self, cards)
        local required = math.ceil(sgs.Self:getHandcardNum() / 2)
        if required == 0 or #cards ~= required then return nil end
        local skill_card = jiuxian4_card:clone()
    ...
    enabled_at_play = function(self, player)
        if player:isKongcheng() or player:hasUsed("#jiuxian4Card") then return false end
        local duel = sgs.Sanguosha:cloneCard("duel", sgs.Card_NoSuit, 0)
        duel:setSkillName(self:objectName())
        local available = duel:isAvailable(player)
        duel:deleteLater()
        return available
    end,
}

jiuxian4 = sgs.CreateTriggerSkill{
    name = "jiuxian4",
    events = {sgs.Damage},
    view_as_skill = jiuxian4VS,

    can_trigger = function(self, event, room, player, data)
        local damage = data:toDamage()
        if not (damage.from and damage.from:isAlive() and damage.from:hasSkill(self:objectName())) then
            return ""
        end
        if not (damage.card and damage.card:isKindOf("Duel")
            and damage.card:getSkillName() == self:objectName()) then
            return ""
        end
        if not (damage.to and damage.to:isAlive()) then return "" end

        for _, target in sgs.qlist(room:getOtherPlayers(damage.to)) do
            if target:isWounded() and damage.to:inMyAttackRange(target) then
                return self:objectName(), damage.from:objectName()
            end
        end
        return ""
    ...
    on_cost = function(self, event, room, player, data, ask_who)
        ...
        local target = room:askForPlayerChosen(ask_who, candidates, self:objectName(),
            "@jiuxian4-recover:" .. damage.to:objectName(), true)
        if not target then return false end
        ask_who:setTag("jiuxian4_recover", sgs.QVariant(target:objectName()))
        room:broadcastSkillInvoke(self:objectName(), ask_who)
        return true
    end,

    on_effect = function(self, event, room, player, data, ask_who)
        local target_name = ask_who:getTag("jiuxian4_recover"):toString()
        ask_who:removeTag("jiuxian4_recover")
...
chenyong = sgs.CreateTriggerSkill{
    name = "chenyong",
    ...
    on_record = function(self, event, room, player, data)
        ...
        if event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_RoundStart then
            for type_id = sgs.Card_TypeBasic, sgs.Card_TypeEquip do
                room:setPlayerMark(player, "chenyong_type_" .. tostring(type_id), 0)
            end
        elseif event == sgs.CardUsed and player:getPhase() ~= sgs.Player_NotActive then
            local use = data:toCardUse()
            if use.from and use.from:objectName() == player:objectName() and use.card then
                local type_id = use.card:getTypeId()
                if type_id == sgs.Card_TypeBasic or type_id == sgs.Card_TypeTrick
                    or type_id == sgs.Card_TypeEquip then
                    room:setPlayerMark(player, "chenyong_type_" .. tostring(type_id), 1)
                end
            end
        end
    end,
    ...
    can_trigger = function(self, event, room, player, data)
        ...
        local count = 0
        for type_id = sgs.Card_TypeBasic, sgs.Card_TypeEquip do
            if player:getMark("chenyong_type_" .. tostring(type_id)) > 0 then
                count = count + 1
            end
        end
        return count > 0 and self:objectName() or ""
    end,
    ...
    on_effect = function(self, event, room, player, data)
        ...
        if count > 0 then player:drawCards(count, self:objectName()) end
        return false
    end,
}

chentai:addSkill(jiuxian4)
...
sgs.LoadTranslationTable{
    ...
    ["#chentai"] = "救陷陈冰",
    ["jiuxian4"] = "救陷",
    [":jiuxian4"] = "出牌阶段限一次，你可以重铸一半（向上取整）手牌，视为使用一张【决斗】。你使用此【决斗】对目标造成伤害后，你可以令其攻击范围内的一名其他角色回复1点体力。",
    ["jiuxian4Card"] = "救陷",
    ["@jiuxian4-recover"] = "救陷：你可以令 %src 攻击范围内的一名其他角色回复1点体力",
    ["chenyong"] = "沉勇",
    [":chenyong"] = "结束阶段，你可以摸X张牌（X为你本回合使用过的牌的类别数）。",
}
