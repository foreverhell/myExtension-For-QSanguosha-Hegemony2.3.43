...
jiezhoutai = sgs.General(testToFix, "zhoutai", "wu", 4, true, false, true)
jiezhoutai:addCompanion("sunquan")
jiezhoutai:addCompanion("jiangqin")
...
jiehengjiang_draw = sgs.CreateTriggerSkill{
    ...
    on_cost = function(self, event, room, player, data, skill_owner)
		room:broadcastSkillInvoke("hengjiang", skill_owner)
		return true
    end,
...
jiehengjiang_fail = sgs.CreateTriggerSkill{ --防止被铁骑后没清掉标记
...
jiezhiren = sgs.CreateTriggerSkill{
	...
	on_effect = function(self, event, room, player, data)
		...
		elseif choice == "jiezhiren_discardEquip" then
			local targets = sgs.SPlayerList()
			for _, p in sgs.qlist(room:getOtherPlayers(player)) do
...
sgs.LoadTranslationTable{
	...
	["jiezhiren_guanxing"] = "观星",
	["jiezhiren_discardEquip"] = "弃置一名女性角色的装备牌",
}
...
jieyinbing_damaged = sgs.CreateTriggerSkill{
	...
	on_cost = function(self, event, room, player, data, skill_owner)
		room:broadcastSkillInvoke("yinbing", skill_owner)
		return true
	end,
		...
		if skillTriggerable(player, self:objectName()) and not player:hasFlag("jienuzhanUsed") then
			...
			elseif event == sgs.CardUsed then
				...
				if use and use.card and use.card:isKindOf("Slash") then
					...
					and player:getPhase() == sgs.Player_Play and (use.card:getSkillName() == "jiange" or use.card:getSkillName() == 
					"wusheng") then
						return self:objectName()
					end
				end
...
sgs.LoadTranslationTable{
    ["jieguanyu"] = "关羽",
    ["jienuzhan"] = "怒斩",
	[":jienuzhan"] = "锁定技，每回合限一次，你使用锦囊牌转化的【杀】无次数限制或者装备牌转化的【杀】造成伤害时伤害+1。",
...
jielieren = sgs.CreateTriggerSkill{
	name = "jielieren",
	events = {sgs.TargetChosen, sgs.CardFinished},
	can_trigger = function(self, event, room, player, data)
		...
		elseif skillTriggerable(player, self:objectName()) and event == sgs.CardFinished then
			if player:hasFlag("jielieren2quick") then
				room:setPlayerFlag(player, "-jielieren2quick")
			end
		end
		return false
	end,
	...
	on_effect = function(self, event, room, skill_target, data, player)
		room:setPlayerFlag(player, "jielieren2quick") --发动技能过快会导致闪退，只能让AI多目标时只发动一次
...
jiejiezi = sgs.CreateTriggerSkill{
	...
	can_trigger = function(self, event, room, player, data)
		if player and player:isAlive() and event == sgs.EventPhaseSkipping then
			...
			if skill_owners:isEmpty() then return false end
            for _, skill_owner in sgs.qlist(skill_owners) do
				if skill_owner:getMark("jiejiezi_draw") < 2 and change.to == sgs.Player_Draw and skill_owner ~= player and
				skillTriggerable(skill_owner, self:objectName()) then
					table.insert(skill_list, self:objectName())
					table.insert(name_list, skill_owner:objectName())
				end
			end
	...
	can_trigger = function(self, event, room, player, data)
		if player and player:isAlive() and event == sgs.TargetChosen then
			...
			if use.card and use.card:getTypeId() == sgs.Card_TypeTrick and use.to:length() > 1 then
				...
				local skill_owners = room:findPlayersBySkillName(self:objectName())
				for _, skill_owner in sgs.qlist(skill_owners) do
					if skillTriggerable(skill_owner, self:objectName()) and skill_owner:getMark("@jiefenwei") > 0 and
					not skill_owner:hasFlag("nojiefenwei") then
						table.insert(skill_list, self:objectName())
						table.insert(name_list, skill_owner:objectName())
					end
				end
		...
		elseif player and player:isAlive() and event == sgs.CardFinished then
			local skill_owners = room:findPlayersBySkillName(self:objectName())
			for _, skill_owner in sgs.qlist(skill_owners) do
				if skillTriggerable(skill_owner, self:objectName()) and skill_owner:hasFlag("nojiefenwei") then
					room:setPlayerFlag(skill_owner, "-nojiefenwei")
				end
			end
		end
		return false
...
sgs.LoadTranslationTable{
    ["jieganfuren"] = "甘夫人",
    ["jieshushen"] = "淑慎",
	[":jieshushen"] = "当你回复1点体力或一次性失去至少两张牌后，你可以选择一名其他角色，令其摸一张牌，若其没有手牌，改为摸两张牌。",
}

jiebuqu = sgs.CreateTriggerSkill{
    name = "jiebuqu",
    events = {sgs.Dying},
    frequency = sgs.Skill_Compulsory,
    can_trigger = function(self, event, room, player, data)
    ...
    on_effect = function(self, event, room, player, data)
        ...
        if is_unique then --若”创“牌堆没有这种花色，放进去
            ...
            player:addToPile("scars", card_ids, true)
...
jiefenji = sgs.CreateTriggerSkill{
    name = "jiefenji",
    events = {sgs.CardsMoveOneTime},
    can_trigger = function(self, event, room, player, data)
...
jiezhoutai:addSkill(jiefenji)
jiezhoutai:addSkill(jiebuqu)
