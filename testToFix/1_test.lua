...
sgs.LoadTranslationTable{
    ["jielvfan"] = "吕范",
    ["jiediaodu"] = "调度",
	[":jiediaodu"] = "每回合首次有与你势力相同的角色使用装备牌时，其可以摸一张牌。出牌阶段开始时，你可以获得一名与你势力相同的角色装备区的一张牌，然后你可以将此牌交给另一名角色。",
...
jiejianglveCard = sgs.CreateSkillCard{ --目前只能写成先亮将再发动原来的“将略”
	...
	on_use = function(self, room, source, targets)
		...
		room:acquireSkill(source, "jianglve", true, true)
	...
	can_trigger = function(self, event, room, player, data)
		...
		if ((player and player:isAlive()) or (use.from and use.from:isAlive())) then
			...
			for _, skill_owner in sgs.qlist(skill_owners) do
				...
				if skill_owner:hasFlag("FirstTarget") or event ~= sgs.TargetConfirmed then return false end
	...
	on_effect = function(self, event, room, player, data)
		...
		elseif event == sgs.EventPhaseStart and player:getPhase() == sgs.Player_Start then
			...
			for i = 1, cards_len do
				...
				local card = sgs.Sanguosha:getCard(card_id)
				room:moveCardTo(card, player, target, sgs.Player_PlaceHand, 
