    ...
    on_effect = function(self, event, room, player, data)
		...
		if target and target:isAlive() then
			...
			if choice == "fillhandcard" then
                ...
                target:fillHandCards(damagedTarget:getMaxHp(), self:objectName())
...
shangshi = sgs.CreateMasochismSkill{
	...
	on_damaged = function(self, player, damage)
        ...
        if choice == "discard" then
			--invoke = room:askForDiscard(player, "shangshi_discard", 1, 1, false, true, "@shangshi-discard")
			invoke = room:askForDiscard(player, self:objectName(), 1, 1, false, true, "@shangshi-discard")
    ...
    on_effect = function(self, event, room, player, data, skill_owner)
		...
		if judge:isGood() and skill_owner:isAlive() then
			...
			if not targets:isEmpty() then
				...
				--local target = room:askForPlayerChosen(skill_owner, targets, "naman_target", prompt, true, true)
				local target = room:askForPlayersChosen(skill_owner, targets, "naman_target", 0, 1, prompt, true)--改成这个是为了能够取消选择
				skill_owner:removeTag("NamanUsedata")
				if not target:isEmpty() then
					room:doAnimate(1, skill_owner:objectName(), target:at(0):objectName())
					if use.to:contains(target:at(0)) then
						sgs.Room_cancelTarget(use, target:at(0))
					else
						use.to:append(target:at(0))
...
qinzhong = sgs.CreateTriggerSkill{
	...
	exchangeDeputyGeneral = function(player1, player2)
...
zhaofu = sgs.CreateTriggerSkill{
	...
	on_effect = function(self, event, room, player, data)
		...
		if not target_to:isEmpty() then
			local target = room:askForPlayerChosen(player, target_to, self:objectName(), "@zhaofu1", true, true)
			if target then
				room:setPlayerMark(target, "@reward", 1)
				room:askForDiscard(player, self:objectName(), 1, 1, false, true, "@zhaofu1_discard")
			end
		end
		return false
	end
...
sgs.LoadTranslationTable{
	...
	["qinzhong"] = "亲重",
	[":qinzhong"] = "副将技，回合开始时，你可以和一名与你势力相同的其他角色交换副将（可以交换未明置的副将）。",
	["zhaofu"] = "招附",
	[":zhaofu"] = "出牌阶段开始时，若场上不足三个“赏”，你可以弃置一张牌，令一名其他角色获得一个“赏”标记。当有“赏”的角色使用基本牌" ..
	"或普通锦囊牌结算完成后，且当前没有未完成的结算时，若你符合此牌的使用条件（无视使用时机），你可弃置其一个“赏”，视为你使用了一张此牌。",

	["@reward"] = "赏",

	["@qinzhong-invoke"] = "亲重：与一名势力相同的角色交换副将",
	["@zhaofu1"] = "是否使用“招附”，弃置一张牌令一名其他角色获得“赏”",
	["@zhaofu1_discard"] = "招附：弃置一张牌",
        ...
        if event == sgs.CardsMoveOneTime then
			...
			if #ids > 0 then
				...
				if to_choose:isEmpty() then
					local x = 0
					for _, p in sgs.qlist(all_players) do
						if not p:isFriendWith(player) and p:getHp() >= x and p:hasShownOneGeneral() then
							if p:getHp() > x then
								x = p:getHp()
		...
		if event == sgs.EventPhaseEnd then
			...
			if player:getMark("haokuitransformUsed") == 0 then
				...
				if not to_choose:isEmpty() then
                    ...
                    if to then--and room:askForChoice(to, "transform_haokui", "yes+no", data, "@transform-ask:::haokui") == "yes" then
...
sgs.LoadTranslationTable{
	...
	["haokui"] = "豪魁",
	[":haokui"] = "出牌阶段开始时，你可摸两张牌。若如此做，在此回合的弃牌阶段：当有牌置入弃牌堆时，你将这些牌交给一名与你势力不同的角色（优先大势力，其次体力值最高的角色）；"..
		"若没有牌进入弃牌堆，你可暗置该武将牌，然后令一名与你势力相同的角色变更副将。",
	["xushi"] = "虚实",
	[":xushi"] = "当其他角色使用的牌仅指定了你为目标时，若该武将牌处于暗置状态，你可明置此武将牌，取消之，然后你可以令其弃置一张牌。",
