...
sgs.ai_skill_invoke.jieshengxi = function(self, data)
...
sgs.ai_skill_discard.jieyicheng = function(self, discard_num, min_num, optional, include_equip)
...
sgs.ai_skill_choice.jieyicheng = "yes"
...
local function will_discard_zhendu(self)
	...
	elseif self:isEnemy(current) or self:isWeak(current) then
		...
		if self:isFriend(current) then return -1 end --是队友则不毒
